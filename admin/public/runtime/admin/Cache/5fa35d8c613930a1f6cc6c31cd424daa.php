...
<!-- 	<?php if(is_array($menus)): $k = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu_group): ++$k;$mod = ($k % 2 )?><dl class="menu <?php if($k > 1): ?>menu-hide<?php endif; ?>">
	...
	<?php if(is_array($menus)): $k = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu_group): ++$k;$mod = ($k % 2 )?><div class="nav ">
			<div class="navlist hasmore <?php if($k > 0): ?>a<?php endif; ?> active">
				<a class="" href="javascript:void(0)" >
				<i class="iconfont left basicdata"><b class="bg-success"></b></i>
				<span><?php echo ($menu_group["name"]); ?></span>
				<i class="iconfont right"></i>
				<div class="blank0"></div>
				</a>
				<div class="navtwo">
				<?php if(is_array($menu_group["nodes"])): foreach($menu_group["nodes"] as $key=>$node): ?><a class="linkto " href="javascript:void(0)" data-href="<?php echo u($node["module"]."/".$node["action"]);?>">
