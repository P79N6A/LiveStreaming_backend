...
$distribution_cfg = array(
    ...
    "OSS_DOMAIN"	=>	"http://kehu990.oss-cn-beijing.aliyuncs.com",  //远程存储域名
	"OSS_DOMAIN_HTTPS"	=>	"",  //远程https存储域名
    "OSS_FILE_DOMAIN"	=>	"http://kehu990.oss-cn-beijing.aliyuncs.com",	//远程存储文件域名(主要指脚本与样式)
    "OSS_BUCKET_NAME"	=>	"kehu990", //针对阿里oss的bucket_name
    "OSS_ACCESS_ID"	=>	"LTAI25CzwuDFBYsA",
    "OSS_ACCESS_KEY"	=>"0PUOyEGn6MGK4JGnMFc7MJyCg2TQ8O",
	"OSS_ENDPOINT"  =>  "kehu990.oss-cn-beijing.aliyuncs.com",  //OSS的endpoint
	"OSS_ENDPOINT_WITH_BUCKET_NAME"  =>  true, //域名中是否已带有bucket属性，一般为三级域名将有此特性
	"NEW_OSS"	=>	true,  //使用新版OSS功能，设置为 true
	"OSS_NO_SAVE_LOCALHOST"	=>	1,  //上传OSS是否保存图片在本地，不保存：0 ；保存：1
    ...
    "AES_EXTRA_FUN" => array(
        'app#init', 'login#do_login', 'user#usersig', 'login#wx_login', 'login#qq_login', 'login#sina_login', 'login#visitors_login', 'login#send_mobile_verify', 'login#do_update', 'app#aliyun_sts', 'login#is_user_verify', 'settings#mobile_binding', 'settings#mobile_login'
    )
);
//关于分布式配置
