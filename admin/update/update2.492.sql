...
MODIFY COLUMN `coins` int(11) NOT NULL DEFAULT '0' COMMENT '消费游戏币';
...
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8 COMMENT='子房间表';
ALTER TABLE `%DB_PREFIX%video`
ADD COLUMN `is_childroom`  tinyint(1) NULL DEFAULT 0 COMMENT '是否为子房间类型；0为普通直播，1为子房间类型' AFTER `classified_id`;
CREATE TABLE `%DB_PREFIX%child_room_viewer` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='子房间观众信息表';
...
ALTER TABLE `%DB_PREFIX%family_join`
ADD COLUMN `apply_type` tinyint(1) NOT NULL DEFAULT 0 COMMENT '0 加入申请 1 退出申请';
