...
ADD COLUMN `iap_diamonds`  int(11) NULL DEFAULT 0 COMMENT '苹果支付获取秀豆';
...
INSERT INTO `%DB_PREFIX%m_config` VALUES ('', 'mission_name', '每日在线任务奖励标题', '基础配置', '免费领取10秀豆', '0', '0', null, null, '每日在线任务奖励标题');
INSERT INTO `%DB_PREFIX%m_config` VALUES ('', 'mission_desc', '每日在线任务奖励说明', '基础配置', '等级越高免费领取秀豆越多', '0', '0', null, null, '每日在线任务奖励说明');
...
INSERT INTO `%DB_PREFIX%mission` VALUES ('1', '领取10秀豆', '', '10', '0', '1', '1', '1');
INSERT INTO `%DB_PREFIX%mission` VALUES ('2', '领取20秀豆', null, '20', '10', '2', '1', '1');
INSERT INTO `%DB_PREFIX%mission` VALUES ('3', '领取30秀豆', null, '30', '20', '3', '1', '1');
...
INSERT INTO `%DB_PREFIX%m_config` (`code`, `title`, `group_id`, `val`, `type`, `sort`, `value_scope`, `title_scope`, `desc`) VALUES ('switch_public_pay', '是否开启公屏付费', '付费直播配置', '0', 4, 167, '0,1', '否,是', '是否开启公屏付费 0关闭，1开启');
INSERT INTO `%DB_PREFIX%m_config` (`code`, `title`, `group_id`, `val`, `type`, `sort`, `value_scope`, `title_scope`, `desc`) VALUES ('public_pay', '公屏收费价格', '付费直播配置', '0', 0, 170, '', '', '(秀豆)公屏收费价格');
...
INSERT INTO `%DB_PREFIX%m_config` (`id`, `code`, `title`, `group_id`, `val`, `type`, `sort`, `value_scope`, `title_scope`, `desc`) VALUES ('', 'change_name', '更改昵称收费值', '基础配置', '100', '0', '0', '', '', '更改昵称收费值，用户可免费更改一次昵称（秀豆）');

ALTER TABLE `%DB_PREFIX%user`
ADD COLUMN `is_change_name` int(1) NOT NULL DEFAULT '0' COMMENT '是否修改过昵称（1=是，0否）';

ALTER TABLE `%DB_PREFIX%video`
ADD COLUMN `public_screen` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否为公屏收费 0 否； 1 是；';

ALTER TABLE `%DB_PREFIX%video_history`
ADD COLUMN `public_screen` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否为公屏收费 0 否； 1 是；';

ALTER TABLE `%DB_PREFIX%live_pay_log`
ADD COLUMN `pay_type` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否为公屏收费记录 0 否； 1 是；';
...
CREATE TABLE `%DB_PREFIX%video_check` (
  ...
  `sort_num` int(10) NOT NULL DEFAULT '0' COMMENT 'sort_init + share_count * 分享权重 + like_count * 点赞权重 + fans_count * 关注权重 + sort * 排序权重 + ticket(本场收到的秀票) * 秀票权重',
  ...
  `sort_init` int(10) NOT NULL DEFAULT '0' COMMENT 'sort_init(初始排序权重) = (用户可提现秀票：fanwe_user.ticket - fanwe_user.refund_ticket) * 保留秀票权重+ 直播/回看[回看是：0; 直播：9000000000 直播,需要排在最上面 ]+ fanwe_user.user_level * 等级权重+ fanwe_user.fans_count * 当前有的关注数权重',
  ...
  `live_fee` int(11) NOT NULL COMMENT '付费直播 收取多少费用； 每分钟收取多少秀豆，主播端设置',
...
INSERT INTO `%DB_PREFIX%m_config` VALUES (null, 'pc_live_fee', 'PC端付费直播收费', '付费直播配置', '', '0', '12', null, null, '(秀豆)PC付费直播默认值，需不低于按场付费最低收费，不高于按场付费最高收费');
...
ALTER TABLE `%DB_PREFIX%user`
ADD COLUMN `distribution_url` varchar(100) NOT NULL COMMENT '分销分享链接';

ALTER TABLE `%DB_PREFIX%user`
ADD COLUMN `qr_code` varchar(100) NOT NULL COMMENT '分销分享二维码图片';

INSERT INTO `%DB_PREFIX%m_config` (`code`, `title`, `group_id`, `val`, `type`, `sort`, `value_scope`, `title_scope`, `desc`) VALUES ('diamond_ticket_rate', '秀豆_秀票兑换比例', '分销模块', '0.5', 0, 0, '0,1', 'NULL', '秀豆兑换成秀票的比例');

INSERT INTO `%DB_PREFIX%m_config` (`code`, `title`, `group_id`, `val`, `type`, `sort`, `value_scope`, `title_scope`, `desc`) VALUES ('distribution_recharge_rate', '分销充值抽成比例', '分销模块', '0.1', 0, 0, '0,1', 'NULL', '下级成员充值，上级成员抽成秀票的比例');

ALTER TABLE `%DB_PREFIX%society`
ADD COLUMN `society_rank` int(10) NOT NULL DEFAULT 0 COMMENT '排序';

UPDATE `%DB_PREFIX%conf` SET `value`='2.46' WHERE (`name`='DB_VERSION');
...
INSERT INTO `%DB_PREFIX%m_config` (`code`, `title`, `group_id`, `val`, `type`, `sort`, `value_scope`, `title_scope`, `desc`) VALUES ('diamond_to_ticket_rate', '秀豆_秀票兑换比例', '分销模块', '0.5', 0, 95, '0,1', 'NULL', '秀豆兑换成秀票的比例(0-1之间的小数),如10个秀豆兑换成秀票数:10*0.5=5个秀票');

INSERT INTO `%DB_PREFIX%m_config` (`code`, `title`, `group_id`, `val`, `type`, `sort`, `value_scope`, `title_scope`, `desc`) VALUES ('distribution_recharge_rate', '分销充值抽成比例', '分销模块', '0.1', 0, 96, '0,1', 'NULL', '下级成员充值，上级成员抽成秀票的比例(0-1之间的小数),如下级成员充值量转为秀票价值为100，则上级抽成秀票数:100*0.1=10个秀票');


CREATE TABLE `%DB_PREFIX%child_room` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'id',
  `child_id` int(11) DEFAULT NULL COMMENT '子房间直播间id',
  `parent_id` int(11) DEFAULT NULL COMMENT '主房间id',
  `is_delete` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否删除；1:删除;0:未删除;',
  PRIMARY KEY (`id`),
  KEY `idx_c_001` (`parent_id`,`is_delete`,`child_id`) USING BTREE
) COMMENT='子房间表';

CREATE TABLE `%DB_PREFIX%child_room_viewer` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `child_room` int(11) NOT NULL COMMENT '子房间id',
  `user_id` int(11) NOT NULL COMMENT '观众id',
  `is_delete` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否删除；0为否，1为删除',
  PRIMARY KEY (`id`)
) COMMENT='子房间观众信息表';
...
INSERT INTO `%DB_PREFIX%m_config` (`code`, `title`, `group_id`, `val`, `type`, `sort`, `value_scope`, `title_scope`, `desc`) VALUES ('bm_point', '不可提现提示语', '提现设置', '代理会员不可提现，请联系平台', 0, 160, NULL, NULL, '代理会员不可提现提示编辑');

UPDATE `%DB_PREFIX%conf` SET `value`='2.491' WHERE (`name`='DB_VERSION');

UPDATE `%DB_PREFIX%m_config` SET `group_id`='第三方帐户', `sort`='4' WHERE (`code`='open_visitors_login');
...
ADD COLUMN `diamonds`  decimal(13,2) NOT NULL COMMENT '邀请奖励的秀豆';
...
UPDATE `%DB_PREFIX%m_config` SET `title`='邀请奖励秀豆', `group_id`='分享设置', `desc`='邀请奖励功能 0代表关闭；大于0的整数代表开启,并且奖励对应数量的秀豆' WHERE (`code`='reward_point_diamonds');

CREATE TABLE `%DB_PREFIX%child_room_account` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `p_user_id` int(11) NOT NULL COMMENT '主账号ID',
  `c_user_id` varchar(255) NOT NULL COMMENT '子账号ID',
  `room_title` varchar(255) DEFAULT NULL COMMENT '房间名称',
  `video_code` varchar(255) DEFAULT NULL COMMENT '验证码',
