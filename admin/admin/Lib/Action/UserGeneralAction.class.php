...
//        $data['is_authentication'] = array('in',array(0,1,3));
    ...
    public function share_distribution_log()
    {
        $common = new UserCommon();
        $data = $_REQUEST;
        $common->share_distribution_log($data);
    }
    ...
    public function edit_wawa()
    {
        $common = new UserCommon();
        $data = $_REQUEST;
        $common->edit_wawa($data);
    }
...
//        $data['is_authentication'] = array('in',array(0,1,3));
    ...
    public function weixin_distribution()
    {
        ...
        $model = M('weixin_distribution');
        if ($_POST) {
            $is_top = intval($_REQUEST['is_top']);
            $first_rate = intval($_REQUEST['first_rate']);
            $second_rate = intval($_REQUEST['second_rate']);
            ...
            if ($second_rate > 100 || $second_rate < 0) {
                $this->ajax_return(array(
                    'status' => 0,
                    'error' => '二级分成比例错误'
                ));
            }
            $data = compact('first_rate', 'second_rate', 'user_id');
            if ($is_top) {
                $data['pid'] = 0;
                $data['topid'] = $user_id;
            }
            ...
            $user_info['pid'] = intval($user_info['pid']);
            $this->assign("user_info", $user_info);
            $this->display();
        }
    }
    ...
    public function game_distribution()
    {
        ...
        if ($_POST) {
            ...
            $where = array('id' => $user_id);
            $user_model = M('user');
            $data = ['game_distribution1' => $game_distribution1, 'game_distribution2' => $game_distribution2];
            if (defined('GAME_DISTRIBUTION_TOP') && GAME_DISTRIBUTION_TOP && $_REQUEST['is_top']) {
                $data['game_distribution_top_id'] = $user_id;
            }
            $res = $user_model->where($where)->save($data);
            if ($res) {
                fanwe_require(APP_ROOT_PATH . 'mapi/lib/redis/BaseRedisService.php');
                fanwe_require(APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php');
                $user_redis = new UserRedisService();
    ...
    //禁私密
    public function forbid_private()
    {
        $id = intval($_REQUEST['user_id']);
        $open_private = $GLOBALS['db']->getRow("select open_private from " . DB_PREFIX . "user where id =" . $id);
        $where = 'id=' . $id;
        require_once APP_ROOT_PATH . 'mapi/lib/redis/BaseRedisService.php';
        require_once APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php';
        $user_redis = new UserRedisService();
        ...
        $data['open_private'] = 1;
        $info = '';
        if ($open_private['open_private'] == 1) {
            $data['open_private'] = 0;
            $info = '取消';
        }
        ...
        if ($list !== false) {
            $user_redis->update_db($id, $data);
            save_log("用户" . $id . $info . "禁私密" . "成功", 1);
            $this->success($info . '禁私密成功', 1);

        } else {
            $this->error($info . '禁私密成功', 1);
        }

    }
    //禁连麦
    public function forbid_lianmai()
    {
        $id = intval($_REQUEST['user_id']);
        $open_lianmai = $GLOBALS['db']->getRow("select open_lianmai from " . DB_PREFIX . "user where id=" . $id);
        $where = 'id=' . $id;
        require_once APP_ROOT_PATH . 'mapi/lib/redis/BaseRedisService.php';
        require_once APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php';
        $user_redis = new UserRedisService();
        $data['open_lianmai'] = 1;
        $info = '';
        if ($open_lianmai['open_lianmai'] == 1) {
            $data['open_lianmai'] = 0;
            $info = '取消';
        }
        $list = $GLOBALS['db']->autoExecute(DB_PREFIX . "user", $data, 'UPDATE', $where);
        if ($list !== false) {
            $user_redis->update_db($id, $data);
            save_log("用户" . $id . $info . "禁连麦成功", 1);
            $this->success($info . '禁连麦成功', 1);

        } else {
            save_log("用户" . $id . $info . "禁连麦失败", 1);
            $this->error($info . '禁连麦成功', 1);
        }
    }

    //禁公会
    public function forbid_sociaty()
    {
        $id = intval($_REQUEST['user_id']);
        $open_lianmai = $GLOBALS['db']->getRow("select open_sociaty from " . DB_PREFIX . "user where id=" . $id);
        $where = 'id=' . $id;
        require_once APP_ROOT_PATH . 'mapi/lib/redis/BaseRedisService.php';
        require_once APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php';
        $user_redis = new UserRedisService();
        $data['open_sociaty'] = 1;
        $info = '';
        if ($open_lianmai['open_sociaty'] == 1) {
            $data['open_sociaty'] = 0;
            $info = '取消';
        }
        $list = $GLOBALS['db']->autoExecute(DB_PREFIX . "user", $data, 'UPDATE', $where);
        if ($list !== false) {
            $user_redis->update_db($id, $data);
            save_log("用户" . $id . $info . "禁公会成功", 1);
            $this->success($info . '禁公会成功', 1);

        } else {
            save_log("用户" . $id . $info . "禁公会失败", 1);
            $this->error($info . '禁公会失败', 1);
        }
    }
    ...
    //禁分销
    public function forbid_distribution()
    {
        $id = intval($_REQUEST['user_id']);
        $distribution_ability = $GLOBALS['db']->getOne("select distribution_ban from " . DB_PREFIX . "user where id =" . $id);
        $where = 'id=' . $id;
        require_once APP_ROOT_PATH . 'mapi/lib/redis/BaseRedisService.php';
        require_once APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php';
        $user_redis = new UserRedisService();

        if ($distribution_ability == 1) {
            $data['distribution_ban'] = 0;
            $info = '取消';
        } else {
            $data['distribution_ban'] = 1;
            $info = '';
        }
        ...
        if ($list !== false) {
            $user_redis->update_db($id, $data);
            $this->success($info . '禁分销成功', 1);
        } else {
            $this->error($info . '禁分销失败', 1);
        }
    }
    ...
    public function number()
    {
        ...
        $user_info = array();
        if ($user_id) {
            $sql_str = "select id,allow_max_number from " . DB_PREFIX . "user where id = " . $user_id;
    ...
    {
        $user_id = intval($_REQUEST['id']);
        $allow_max_number = intval($_REQUEST['allow_max_number']);
        // if ($allow_max_number > 2147483647) {
        //     return false;
        // }
        $where = 'id=' . $user_id;
        $data['allow_max_number'] = $allow_max_number;
        require_once APP_ROOT_PATH . 'mapi/lib/redis/BaseRedisService.php';
        require_once APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php';
        $user_redis = new UserRedisService();
        $list = $GLOBALS['db']->autoExecute(DB_PREFIX . "user", $data, 'UPDATE', $where);
        if ($list !== false) {
            $user_redis->update_db($user_id, $data);
            save_log("用户" . $user_id . "直播间最大真实人数变更为" . $allow_max_number . "成功", 1);
            $this->success(L("UPDATE_SUCCESS"));

        } else {
            save_log("用户" . $user_id . "直播间最大真实人数变更为" . $allow_max_number . "失败", 1);
    ...
    public function svideo()
    {
        ...
        $user_info = array();
        if ($user_id) {
            $sql_str = "select id,open_svideo,allow_svideo_number from " . DB_PREFIX . "user where id = " . $user_id;
            $user_info = $GLOBALS['db']->getRow($sql_str);
        }
        $this->assign("user_info", $user_info);
        $this->display();
    }
    public function modify_svideo()
    {
        $user_id = intval($_REQUEST['id']);
        $open_svideo = intval($_REQUEST['open_svideo']);
        $allow_max_number = intval($_REQUEST['allow_svideo_number']);
        // if ($allow_max_number > 2147483647) {
        //     return false;
        // }
        $where = 'id=' . $user_id;
        $data['open_svideo'] = $open_svideo;
        $data['allow_svideo_number'] = $allow_max_number;
        require_once APP_ROOT_PATH . 'mapi/lib/redis/BaseRedisService.php';
        require_once APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php';
        $user_redis = new UserRedisService();
        $list = $GLOBALS['db']->autoExecute(DB_PREFIX . "user", $data, 'UPDATE', $where);
        ...
        if ($list !== false) {
            $user_redis->update_db($user_id, $data);
            save_log("用户" . $user_id . $info . "发布小视频,小视频数量变更为" . $allow_max_number . "成功", 1);
            $this->success(L("UPDATE_SUCCESS"));

        } else {
            save_log("用户" . $user_id . $info . "发布小视频,小视频数量变更为" . $allow_max_number . "失败", 1);
            $this->error(L("UPDATE_FAILED"));
        }
    }
