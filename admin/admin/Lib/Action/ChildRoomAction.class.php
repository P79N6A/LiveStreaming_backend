...
class ChildRoomAction extends CommonAction
{
    ...
    {
        $video_list = $GLOBALS['db']->getAll("SELECT v.id,v.user_id FROM " . DB_PREFIX . "video v where v.live_in in (1,3)  order by v.sort");
        $child_id = $GLOBALS['db']->getAll("SELECT child_id FROM " . DB_PREFIX . "child_room ");
        ...
        foreach ($video_list as $k => $v) {
            ...
            $video_list[$k]['nick_name'] = $GLOBALS['db']->getOne("SELECT nick_name FROM " . DB_PREFIX . "user WHERE id =" . $v['user_id']);
            reset($video_list);
        }
    ...
    public function insert_child_room()
    {
        ...
        if ($has_room) {
            admin_ajax_return(array('status' => '0', 'error' => '该用户已有房间'));
        }
        $is_private = false;
        ...
        if ($parent_id) {
            $parent_info = $GLOBALS['db']->getRow("SELECT v.live_in,v.video_vid,v.channelid,v.play_url,v.push_rtmp,v.play_flv,v.play_mp4,v.play_hls,v.group_id FROM " . DB_PREFIX . "video v  where v.live_in in (1,3) and v.id =" . $parent_id);
            if (empty($parent_info)) {
                admin_ajax_return(array('status' => '0', 'error' => '主房间不存在'));
            }
            $data['video_vid'] = $parent_info['video_vid'];
            $data['channelid'] = $parent_info['channelid'];
            $data['push_rtmp'] = $parent_info['push_rtmp'];
        ...
        if ($GLOBALS['db']->affected_rows()) {
            ...
            if ($GLOBALS['db']->autoExecute(DB_PREFIX . "child_room", $child_data, 'INSERT')) {
                ...
                $video_redis->video_online($data['id'], $data['group_id']);
            } else {
                $res['status'] = 0;
                $res['error'] = '关联主房间失败！';
            }
    ...
    public function update_child_room()
    {
        ...
        if ($GLOBALS['db']->autoExecute(DB_PREFIX . "video", $data, 'UPDATE', 'id =' . $room_id)) {
            ...
            if ($GLOBALS['db']->autoExecute(DB_PREFIX . "child_room", $child_data, 'UPDATE', 'child_id =' . $room_id)) {
                ...
                $video_redis->video_online($room_id, $data['group_id']);
            } else {
                $res['status'] = 0;
                $res['error'] = '关联主房间失败！';
            }
