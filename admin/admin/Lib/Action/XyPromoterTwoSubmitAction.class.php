...
class XyPromoterTwoSubmitAction extends CommonAction
{
    ...
    public function update()
    {
        ...
        if (false !== $list) {
            if($data['status'] ==1){
                $GLOBALS['db']->query("update ".DB_PREFIX."bm_promoter set child_count=child_count+1 where id= ".intval($data_info['pid'])."");
            }
    ...
    public function check_user($user_id)
    {
        ...
        //是否有会员
        $user_info=$GLOBALS['db']->getRow("select id,bm_pid,is_effect,nick_name,society_id from ".DB_PREFIX."user where id= ".$user_id." ");
        if(!$user_info){
            $return["status"]=0;
