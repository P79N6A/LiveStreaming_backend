...
class BmUserCommon extends CommonAction{
   ...
   /* public function del_contribution_list($data)
	...
	public function modify_account($data)
	{
        ...
        if ($data_arr['diamonds'] >= 0)
		    save_log("用户".$user_id."秀豆增加".$data_arr['diamonds']."成功",1);
        elseif ($data_arr['diamonds'] < 0){
            save_log("用户".$user_id."秀豆减少".(-$data_arr['diamonds'])."成功",1);
        }
		return true;
	}
...
//        }
        ...
        if($data['years']!=''&&$data['month']!=''){
            $sql_str = "SELECT l.id,l.create_ym,l.to_user_id, l.create_time,l.prop_id,l.prop_name,l.from_user_id,l.create_date,l.num,l.total_ticket,u.nick_name,l.from_ip,l.is_coin
