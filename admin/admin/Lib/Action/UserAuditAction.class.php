...
//		$map['_string']=" member_type = 1 ";
	...
	public function delete() {
		...
		$id = $_REQUEST ['id'];
		//防止该成员是公会长被删除ljz
		$user_info = $GLOBALS['db']->getRow("select society_id,society_chieftain,luck_num from ".DB_PREFIX."user where id=$id");
		if($user_info['society_chieftain']){
		    $this->error('该成员是公会长，请先将该成员公会解散',$ajax);
		}else if($user_info['luck_num']){
			$this->error('账号'.$id.'绑定了靓号'.$user_info['luck_num'].'，请先回收该靓号',$ajax);
		}else{
		    if (isset ( $id )) {
