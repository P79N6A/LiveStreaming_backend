...
//        if(define(DISTRIBUTION_MODULE) && DISTRIBUTION_MODULE == 1){
        ...
        if (defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE) {
            $res = $GLOBALS['db']->getAll("select cc.id,cc.title,cc.is_effect as type from " . DB_PREFIX . "user_classify uc join " . DB_PREFIX . "car_classify cc on cc.id=uc.classify_id where uc.user_id=$id");
            if (empty($res)) {
                $res = [];
            }
            $this->assign('open_car', 1);
            $this->assign('car_classify', $res);
            //$this->assign('user_classify',$res1);
        }

        $this->assign('open_car', 0);
...
//更新家族ID
                ...
                if (defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE) {
                    if ($status == 2) {
                        $GLOBALS['db']->autoExecute(DB_PREFIX . "user_classify", array('is_effect' => 1), 'UPDATE', "user_id=" . $user_id[$i]);
                    }
                }
