...
//        echo $condition;
    ...
    public function edit()
    {
        ...
        $id = intval($_REQUEST['id']);
        $vo = M('Weibo')->find($id);
        if ($vo['type'] == 'imagetext' || $vo['type'] == 'photo' || $vo['type'] == 'goods' || $vo['type'] == 'red_photo') {
            $vo['data'] = unserialize($vo['data']);
            foreach ($vo['data'] as $k1 => $v1) {
                $info_type = $vo['type'] . '_info';
                //        $vo['data'][$k1]['url'] =  get_file_oss_url($v1['url'],$info_type);
                //        $vo['data'][$k1]['url'] =deal_weio_image($v1['url']);
                $vo['data'][$k1]['url'] = get_spec_image($v1['url']);
            }
            $this->assign('data', $vo['data']);
        } else {
            $vo['data'] = get_file_oss_url($vo['data']);
        }
        $this->assign('vo', $vo);
        $this->display();
    }
    ...
    {
        $data = M('Weibo')->create();
        $log_info = M('Weibo')->where("id=" . intval($data['id']))->getField("title");

        //开始验证有效性
        $this->assign("jumpUrl", u('WeiboList' . "/edit", array("id" => $data['id'])));

        // 更新数据
        if ($data['id']) {
            $list = M('Weibo')->save($data);
        } else {
            $data['create_time'] = NOW_TIME;
            $list = M('Weibo')->add($data);
        }
        if (false !== $list) {
            //成功提示
