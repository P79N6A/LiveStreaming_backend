...
class UserGuardAction extends CommonAction
{
    ...
    public function index()
    {
        ...
        $create_time_2 = $_REQUEST['create_time_2'];
        $parameter = '';
        $sql_w = '';
        if (trim($nick_name) != '') {
            $parameter .= "u.nick_name like " . urlencode('%' . trim($nick_name) . '%') . "&";
            $sql_w .= "u.nick_name like '%" . trim($nick_name) . "%' and ";
        }

        if (trim($mobile) != '') {
            $parameter .= "u.mobile like " . urlencode('%' . trim($mobile) . '%') . "&";
            $sql_w .= "u.mobile like '%" . trim($mobile) . "%' and ";
        }
        $create_time_2 = empty($create_time_2) ? to_date(get_gmtime(), 'Y-m-d') : strim($create_time_2);
        $create_time_2 = to_timespan($create_time_2) + 24 * 3600;
        if (trim($create_time_1) != '') {
            $parameter .= "u.create_time between '" . to_timespan($create_time_1) . "' and '" . $create_time_2 . "'&";
            $sql_w .= " (u.create_time between '" . to_timespan($create_time_1) . "' and '" . $create_time_2 . "' ) and ";
        }
        if (intval($id) > 0) {
            $parameter .= "u.id=" . intval($id) . "&";
            $sql_w .= "u.id=" . intval($id);
        }

        $model = D();

        $sql_str = "SELECT *,ticket-refund_ticket as useable_ticket FROM " . DB_PREFIX . "user_guard WHERE 1=1 ";
        $count_sql = "SELECT count(*) as tpcount" .
            " FROM " . DB_PREFIX . "user_guard WHERE (1=1 ";
        $sql_str = "SELECT ug.*,u.nick_name,u.head_image,u.mobile,u.create_time,u.user_level,u.diamonds,u.use_diamonds,u.ticket,(u.ticket-u.refund_ticket) as useable_ticket,u.login_time,u.logout_time,u.login_ip,u.is_effect,u.is_nospeaking,(ug.view_count-ug.old_view_count) as c_view_count  from " . DB_PREFIX . "user_guard as ug LEFT JOIN " . DB_PREFIX . "user as u on u.id = ug.user_id WHERE 1=1 ";
        $count_sql = "SELECT count(*)  as tpcount  from " . DB_PREFIX . "user_guard as ug LEFT JOIN " . DB_PREFIX . "user as u on u.id = ug.user_id  WHERE (1=1  ";
        ...
        if (intval($id) == 0) {
            ...
            $sql_str .= " and " . $sql_w . " 1=1  ";
            $count_sql .= " and " . $sql_w . " and 1=1 )";
        }
//        print_r($sql_str);echo "<hr/>";
        //        print_r($count_sql);echo "<hr/>";
        $voList = $this->_Sql_list($model, $sql_str, "&" . $parameter, 'c_view_count', 0, $count_sql);
        foreach ($voList as $k => $v) {
            $voList[$k]['id'] = $v['user_id'];
            $voList[$k]['nick_name'] = ($v['nick_name']);
            $voList[$k]['head_image'] = get_spec_image($v['head_image']);
            $voList[$k]['color'] = 0;
            if ($voList[$k]['c_view_count'] > 50) {
                $voList[$k]['color'] = 1;
            }
        }
//        print_r($voList);
    ...
    public function closed_prop()
    {
        ...
        if ($data['years'] != '' && $data['month'] != '') {
            $sql_str = "SELECT l.id,l.create_ym,l.to_user_id, l.create_time,l.prop_id,l.prop_name,l.from_user_id,l.create_date,l.num,l.total_ticket,u.nick_name,l.is_coin
            ...
            $total_ticket_sql = "SELECT SUM(l.total_ticket)  as tpcount
                         ...
                         LEFT JOIN " . DB_PREFIX . "user AS u  ON l.from_user_id = u.id" . " LEFT JOIN " . DB_PREFIX . "prop AS v ON l.prop_name = v.name" . "
                         WHERE $where " . "   and " . $sql_w . " 1=1  ";
            $table = DB_PREFIX . 'video_prop_' . date('Ym', NOW_TIME);
        }
...
//        $data['is_authentication'] = array('in',array(0,1,3));
        ...
        if ($_POST) {
            ...
            if ($is_top) {
                ...
                if ($res['topid']) {
                    ...
                    $p = $model->where(['user_id' => $pid])->find();
                    if (!$p['topid']) {
                        $this->ajax_return(array(
                            'status' => 0,
                            'error' => '上级用户未参与分销'
    ...
    public function game_distribution()
    {
        ...
        if ($_POST) {
            ...
            if (defined('GAME_DISTRIBUTION_TOP') && GAME_DISTRIBUTION_TOP) {
                if ($_REQUEST['is_top']) {
                    $data['game_distribution_top_id'] = $user_id;
                } else {
                    $where = array('user_id' => $user_id);
                    $res = $user_model->where($where)->find();
                    if (!$res['game_distribution_top_id']) {
                        $this->ajax_return(array(
                            'status' => 0,
                            'error' => '该用户未参与分销'
                        ));
