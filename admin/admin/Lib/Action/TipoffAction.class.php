...
class TipoffAction extends CommonAction
{
    ...
    public function index()
    {
        ...
        $map[DB_PREFIX . 'tipoff.video_id'] = array('gt', 0);
    ...
    {
        $now = get_gmtime();
        if (trim($_REQUEST['weibo_id']) != '') {
            $map[DB_PREFIX . 'tipoff.weibo_id'] = array('like', '%' . trim($_REQUEST['video_id']) . '%');
        }
        if (trim($_REQUEST['from_user_id']) != '') {
            $user = M("User")->where("nick_name like '%" . trim($_REQUEST['from_user_id']) . "%' ")->findAll();
            $user_arr_id = array();
        ...
        $map[DB_PREFIX . 'tipoff.video_id'] = array('eq', 0);
        $name = $this->getActionName();
        $model = D($name);
        if (!empty($model)) {
            $this->_list($model, $map);
