...
class UserConfirmRefundAction extends CommonAction
{
    ...
    public function refund_confirm()
    {
        ...
        } else if (intval($refund_data['withdrawals_type']) == 3) {
            $alipay_info = M("User")->where("id = " . $refund_data['user_id'])->field('bank_name,branch_name,open_account_num,open_account_name')->find();
            $this->assign("bank_info", $alipay_info);
        }
        $this->assign("refund_data", $refund_data);
        $this->display();
    }
                ...
                if (in_array('3', $is_pay)) {
                    ...
                    if (intval($refund_user['ticket']) - intval($refund_user['refund_ticket']) < intval($refund_data['ticket'])) {
                        ...
                        if ($GLOBALS['db']->affected_rows()) {
                            ...
                            $param['type'] = 1; //类型 0表示充值 1表示提现 2赠送道具 3 兑换秀票
            ...
            if (in_array('3', $is_pay)) {
                ...
                if ($withdrawals_type[$i] == 0) {
                    ...
                    if ($confirm_test) {
                        ...
                        $sql = "select id from " . DB_PREFIX . "user_refund  where id = " . $id[$i] . " and is_pay = 3 ";
                        $user_refund_id = $GLOBALS['db']->getOne($sql);
                        if (intval($user_refund_id)) {
                            //写入用户日志
                            $data = array();
                            $data['ticket'] = intval($ticket[$i]);
                            $data['log_admin_id'] = intval($adm_id);
                            $param['type'] = 1; //类型 0表示充值 1表示提现 2赠送道具 3 兑换秀票
                            $ticket_name = $m_config['ticket_name'] != '' ? $m_config['ticket_name'] : '秀票';
                            $log_msg = '提现' . $ticket[$i] . $m_config['ticket_name'] . '成功';
                            account_log_com($data, $user_id[$i], $log_msg, $param);
        ...
        if ($refund_data && $refund_data['is_pay'] == 1) {
            ...
            if ($refund_data['withdrawals_type'] == 0) {
                ...
                if (wx_withdraw_cash($id)) {
                    ...
                    $sql = "select id from " . DB_PREFIX . "user_refund  where id = " . $id . " and is_pay = 3 ";
                    $user_refund_id = $GLOBALS['db']->getOne($sql);
                    if (intval($user_refund_id)) {
                        //写入用户日志
                        $data = array();
            ...
            } elseif ($refund_data['withdrawals_type'] == 3) {
                //银行卡提现
                //支付成功
                if (intval($_REQUEST['status']) == 1) {
                    $sql = "update " . DB_PREFIX . "user_refund set pay_log='已付款',is_pay = 3,pay_time = " . get_gmtime() . ",confirm_cash_ip='" . get_client_ip() . "' where id = " . $id;
            ...
            } elseif ($refund_data['withdrawals_type'] == 2) {
//公会提现
