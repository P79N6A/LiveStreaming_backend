...
class ConfAction extends CommonAction
{
    ...
    public function mobile()
    {
        ...
        static $society_pattern; //记录公会模式 ljz
        foreach ($config as $k => $v) {
            ...
            if (defined('CHANGE_NAV') && CHANGE_NAV == "wawa") {
                if ($v['group_id'] == '排序权重' || $v['group_id'] == '提现设置') {
                    unset($config[$k]);
                    continue;
                }
                if (in_array($v['code'], array(
                    ...
                    'robot_prop_num',
                    'robot_prop_diamonds',
                    'robot_prop_total_diamonds',
                    'robot_prop_interval',
                    'robot_prop_real_interval',
                    'rank_cache_time',
                    'has_is_authentication',
                    'must_cate',
                    'must_authentication',
                    'svideo_must_authentication',
                    'auto_logistics',
                    'rank_day',
                    'rank_month',
                    'rank_all',
            ...
            if (!(defined('OPEN_MISSION') && OPEN_MISSION == 1) && in_array($v['code'], ['mission_switch', 'mission_money', 'mission_max_times', 'mission_time', 'mission_name', 'mission_desc'])) {
                unset($config[$k]);
                continue;
            }
            //置顶权重
            ...
            if (!defined('OPEN_FAMILY_MODULE') || OPEN_FAMILY_MODULE != 1) {
                if ($v['code'] == 'profit_ratio' || $v['code'] == 'family_profit_platform' || $v['code'] == 'videotime_to_experience' || $v['code'] == 'contribution_to_experience') {
                    unset($config[$k]);
                    continue;
                }
            }
            ...
            if ((!defined('OPEN_SOCIETY_MODULE') || OPEN_SOCIETY_MODULE != 1)) {
                if ($v['code'] == 'society_pattern' || $v['code'] == 'society_platform_earnings') {
                    unset($config[$k]);
                    continue;
                }
            }
            if ($v['code'] == 'society_pattern' && $v['val'] == 1) {
//有抽成模式下才显示 （是否平台支付公会收益）（公会收益比例）
                $society_pattern = 1;
            } elseif ($v['code'] == 'society_pattern' && $v['val'] == 2) {
//(对公结算比例)(主播对公结算比例)
                $society_pattern = 2;
            } elseif ($v['code'] == 'society_pattern' && $v['val'] == 0) {
//关闭上面四个
                $society_pattern = 0;
            }
            //对公结算比例
            ...
            //是否平台支付公会收益
            if (((!defined('OPEN_SOCIETY_MODULE') || OPEN_SOCIETY_MODULE != 1)) && ($v['code'] == 'society_profit_platform')) {
                unset($config[$k]);
                continue;
            }
            //公会收益比例
            if (((!defined('OPEN_SOCIETY_MODULE') || OPEN_SOCIETY_MODULE != 1)) && ($v['code'] == 'society_profit_ratio')) {
                unset($config[$k]);
                continue;
            }
            //直播时长积分转化比
            if (((!defined('OPEN_SOCIETY_MODULE') || OPEN_SOCIETY_MODULE != 1)) && ($v['code'] == 'society_lv_videotime')) {
                unset($config[$k]);
                continue;
            }
            //成员贡献积分转化比
            if (((!defined('OPEN_SOCIETY_MODULE') || OPEN_SOCIETY_MODULE != 1)) && ($v['code'] == 'society_lv_contribution')) {
                unset($config[$k]);
                continue;
            }
            //公会邀请码
            if (((!defined('OPEN_SOCIETY_MODULE') || OPEN_SOCIETY_MODULE != 1)) && ($v['code'] == 'open_society_code')) {
                unset($config[$k]);
                continue;
            }
            //公会名称
            if (((!defined('OPEN_SOCIETY_MODULE') || OPEN_SOCIETY_MODULE != 1)) && ($v['code'] == 'society_list_name')) {
                unset($config[$k]);
                continue;
            }
            //公会名称
            if (((!defined('OPEN_EDU_MODULE') || OPEN_EDU_MODULE != 1)) && in_array($v['code'], ['edu_booking_background', 'edu_booking_title', 'edu_org_background', 'edu_org_title', 'edu_org_list_title', 'edu_video_category', 'edu_deals_category'])) {
                unset($config[$k]);
                continue;
            }
            ...
            if (!(defined('WEIXIN_DISTRIBUTION') && WEIXIN_DISTRIBUTION) && $v['code'] == 'weixin_first_rate') {
                unset($config[$k]);
                continue;
            }
            if (!(defined('WEIXIN_DISTRIBUTION') && WEIXIN_DISTRIBUTION) && $v['code'] == 'weixin_second_rate') {
                unset($config[$k]);
                continue;
            }
            ...
            if (!(defined('GAME_AUTO_START') && GAME_AUTO_START) && $v['code'] == 'auto_time') {
                unset($config[$k]);
                continue;
            }
            if (!defined('ROBOT_PROP') && ROBOT_PROP && in_array($v['code'], 'robot_prop_num', 'robot_prop_diamonds', 'robot_prop_total_diamonds', 'robot_prop_interval', 'robot_prop_real_interval')) {
                unset($config[$k]);
                continue;
            }
            ...
            if (intval(LIVE_PAY_SCENE) == 0) {
                if ($v['code'] == 'live_pay_scene_max' || $v['code'] == 'live_pay_scene_min' || $v['code'] == 'pc_live_fee') {
                    unset($config[$k]);
                    continue;
                }
            }
            ...
            /*if ($v['code'] == 'video_type' || $v['group_id'] == '千秀云') {
            ...
            if ((!defined('PROP_NOTIFY') || PROP_NOTIFY != 1)) {
                if ($v['code'] == 'is_prop_notify') {
                    unset($config[$k]);
                    continue;
                }
            }
            //PC端
            if ((!defined('QK_TREE') || QK_TREE != 1) && $v['group_id'] == '树苗配置') {
                unset($config[$k]);
                continue;
            }
            //移除苹果价格是否与其他支付一致
            ...
            if ($v['code'] == 'system_account_id') {
                $v['value_scope'] = array();
                $v['title_scope'] = array();
                $send_user = M("User")->where("is_admin=1")->findAll();
                foreach ($send_user as $u) {
                    $v['value_scope'][] = $u['id'];
                    $v['title_scope'][] = $u['nick_name'];
                }
            }
        ...
        if (intval(OPEN_REWARD_GIFT) == 1) {
            $award_config = M("AwardConfig")->order("sort asc")->findAll();
            $award_config_arr = array();
            foreach ($award_config as $kk => $vv) {
                if ($vv['type'] == 4) {
                    $vv['value_scope'] = explode(',', $vv['value_scope']);
                    $vv['title_scope'] = explode(',', $vv['title_scope']);
                } else {
                    $vv['value_scope'] = '';
                }
                if ($vv['code'] == 'award_updatetime') {
                    $vv['val'] = to_date($vv['val']);
                }
                if ($vv['code'] == 'amount') {
                    $amount = $vv['val'];
                }
                if ($vv['code'] == 'used_amount') {
                    $used_amount = $vv['val'];
                }
                $award_config_arr[$vv['group_id']][] = $vv;
            }
            $residue = array();
            $residue['code'] = 'residue';
            $residue['title'] = '剩余秀豆';
            $residue['group_id'] = $award_config[0]['group_id'];
            $residue['val'] = intval($amount - $used_amount);
            $residue['type'] = '6';
            $residue['sort'] = '11';
            $residue['value_scope'] = '';
            $residue['title_scope'] = '';
            $residue['desc'] = '（秀豆）下次中奖最高额度';
            $award_config_arr[$award_config[0]['group_id']][] = $residue;
            if ($award_config_arr) {
                $conf = array_merge($conf, $award_config_arr);
            }
        }
    ...
    public function savemobile()
    {
        ...
        foreach ($_POST as $k => $v) {
            ...
            if ((defined('OPEN_LIVE_PAY') && OPEN_LIVE_PAY == 1) && ($k == 'live_pay_num' || $k == 'pc_live_fee' || $k == 'live_pay_rule' || $k == 'live_pay_fee' || $k == 'live_count_down' || $k == 'ticket_to_rate' || $k == 'uesddiamonds_to_score')) {
                if (floor($v) != $v) {
            ...
            if (defined('OPEN_STICK') && OPEN_STICK == 1) {
                if (intval($_POST['top_weight']) < 1) {
                    $this->error("置顶权重值不能小于1");
                }
            }

            //公会配置添加限制ljz
            if (defined('OPEN_SOCIETY_MODULE') && OPEN_SOCIETY_MODULE) {
                if ($k == 'society_public_rate') {
                    if (floatval($v) <= 0 || floatval($v) >= 1) {
                        $this->error("对公结算比例千秀0~1之间");
                    }
                }
                if ($k == 'society_user_rate') {
                    if (floatval($v) <= 0 || floatval($v) >= 1) {
                        $this->error("主播对公结算比例千秀0~1之间");
                    }
                }
            ...
            if (defined('OPEN_LIVE_PAY') && OPEN_LIVE_PAY == 1) {
                if (intval(LIVE_PAY_TIME) == 1) {
                    ...
                    if (intval($_POST['live_pay_max']) != 0 && intval($_POST['live_pay_max']) < intval($_POST['live_pay_min'])) {
                ...
                if (intval(LIVE_PAY_SCENE) == 1) {
                    ...
                    if (intval($_POST['live_pay_scene_max']) != 0 && intval($_POST['live_pay_scene_max']) < intval($_POST['live_pay_scene_min'])) {
                        $this->error("按场收费最低收费参数必须小于按场收费最高收费参数");
                    }
                    if (intval($_POST['pc_live_fee']) > 0) {
                        if (intval($_POST['pc_live_fee']) < intval($_POST['live_pay_scene_min'])) {
                            $this->error("PC端付费直播费率不能小于按场收费最低收费");
                        }
                        if (intval($_POST['live_pay_scene_max']) < intval($_POST['pc_live_fee'])) {
                            $this->error("PC端付费直播费率必须小于按场收费最高收费参数");
                        }
                    }
                }
            }
            ...
            //中奖礼物
            $award_arr = array('is_open_award', 'award_pool_ratio', 'award_ratio', 'award_condition', 'big_prize_limit', 'award_platform_fee', 'award_limit');
            if (in_array($k, $award_arr)) {
                if (intval(OPEN_REWARD_GIFT) == 1) {
                    if ($k == 'big_prize_limit' && $v > 999) {
                        $this->error("大奖条件，数值不能大于999倍");
                    }
                    $res = M("AwardConfig")->where("code='" . $k . "'")->setField("val", trim($v));
                }
            } else {
    ...
    //检查排行榜缓存时间
    public function check_rank_time($data)
    {
    ...
    public function addself_init_version()
    {
        //更新初始化版本号
