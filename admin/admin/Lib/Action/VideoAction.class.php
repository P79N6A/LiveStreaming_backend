 ...
 */
    public function online_index()
    {
        ...
        foreach ($voList as &$value) {
            if (intval($value['live_in']) == 3) {
                ...
                if (defined('CHILD_ROOM') && CHILD_ROOM == 1) {
                    if ($GLOBALS['db']->getOne("SELECT COUNT(id) FROM " . DB_PREFIX . "child_room WHERE child_id=" . $value['id'])) {
                        $value['max_watch'] = "子房间视频不显示";
                    }
                }
            }
        ...
        if (defined('CHILD_ROOM') && CHILD_ROOM == 1) {
            $this->assign('is_child_room', 1);
        } else {
            $this->assign('is_child_room', 0);
        }
        ...
        if (isset($id)) {
            ...
            foreach ($rel_data as $data) {
                if ($data['live_in'] == 3) {
                    ...
                    //同时下线子房间
                    $is_childroom = $GLOBALS['db']->getOne("SELECT COUNT(child_id) FROM " . DB_PREFIX . "child_room WHERE parent_id =" . $data['id']);
                    if (defined('CHILD_ROOM') && CHILD_ROOM == 1 && $is_childroom) {
                        $child_id = $GLOBALS['db']->getAll("SELECT child_id FROM " . DB_PREFIX . "child_room WHERE parent_id =" . $data['id']);
                        if ($child_id) {
                            foreach ($child_id as $item) {
                                $child_ids[] = $item['child_id'];
                            }
                            $child_ids = implode(',', $child_ids);
                            $child_video = $GLOBALS['db']->getAll("SELECT id,user_id,max_watch_number,virtual_watch_number,robot_num,vote_number,group_id,room_type,begin_time,end_time,channelid,cate_id,video_vid FROM " . DB_PREFIX . "video WHERE id in (" . $child_ids . ")");
                            if (!empty($child_video)) {
                                foreach ($child_video as $value) {
                                    $child_res = video_status($value['id'], 1);
                                }
                            }
                        }
                    }
            ...
            if ($video['play_url'] != '') {
                ...
                if (isset($root['vodset'])) {
                    ...
                    foreach ($play_url_list as $k2 => $v2) {
                        foreach ($v2 as $kk => $vv) {
                            //mp4
                            if ($vv['definition'] == 0 && strpos($vv['url'], '.mp4')) {
//原画mp4 播放URL
...
//标清mp4 播放URL
                            ...
                            if ($vv['definition'] == 40 || $vv['definition'] == 30) {
//高清mp4 播放URL
                                $video['mp4_hd_url'] = $vv['url'];
                            }
                            //m3u8
                            if ($vv['definition'] == 0 && strpos($vv['url'], '.m3u8')) {
//原画m3u8 播放URL
                                $video['m3u8_url'] = $vv['url'];
                            }
                            if (($vv['definition'] == 210) && strpos($vv['url'], '.m3u8')) {
//手机m3u8 播放URL
                                $video['m3u8_url'] = $vv['url'];
                            }
                            if (($vv['definition'] == 220) && strpos($vv['url'], '.m3u8')) {
//标清m3u8 播放URL
                                $video['m3u8_sd_url'] = $vv['url'];
                            }
                            if (($vv['definition'] == 230) && strpos($vv['url'], '.m3u8')) {
//高清m3u8 播放URL
                                $video['m3u8_hd_url'] = $vv['url'];
                            }
                            //flv
                            if (strpos($vv['url'], '.flv') && $vv['definition'] == 0) {
//原画flv 播放URL
...
//高清flv 播放URL
                            ...
                            //rtmp
                            if (!strpos($vv['url'], '.flv') && !strpos($vv['url'], '.mp4') && !strpos($vv['url'], '.m3u8') && $vv['definition'] == 0) {
//原画rtmp 播放URL
...
//高清rtmp 播放URL
    ...
    public function push_anchor_all()
    {
        $room_id = $_REQUEST['id'];
        //创建直播推送消息
        $video = $GLOBALS['db']->getRow("select v.title,v.city,u.id as user_id,u.nick_name,u.head_image from " . DB_PREFIX . "video as v left join " . DB_PREFIX . "user as u on u.id=v.user_id where  v.id=" . $room_id);
...
//        elseif($_REQUEST['send_type']==0) {
    ...
    public function end_child_room($id)
    {
        $data = $GLOBALS['db']->getAll("SELECT child_id FROM " . DB_PREFIX . "child_room WHERE parent_id =" . $id);
        if (empty($data)) {
            return;
        }

        $child_ids = implode(',', array_column($data, 'child_id'));
        $child_video = $GLOBALS['db']->getAll("SELECT id,user_id,max_watch_number,virtual_watch_number,robot_num,vote_number,group_id,room_type,begin_time,end_time,channelid,cate_id,video_vid FROM " . DB_PREFIX . "video WHERE id in (" . $child_ids . ")");
        foreach ($child_video as $value) {
            video_status($value['id'], 1);
        }
    }
    ...
    public function set_video_code()
    {
        ...
        $data = M('EduVideoInfo')->where('video_id =' . $room_id)->find();
        $this->assign('room_id', $room_id);
        $this->assign('video', $data);
        $this->display();
    }

    public function modify_video_code()
    {
        if (!defined('CHILD_ROOM') || CHILD_ROOM != 1) {
            $this->error('功能未开启');
        }
        ...
        //开始验证有效性
        $this->assign("jumpUrl", u(MODULE_NAME . "/online_index"));

        //判断是否是直播状态
        $is_live = $GLOBALS['db']->getOne("SELECT COUNT(*) FROM " . DB_PREFIX . "video WHERE live_in in (1,3) AND id =" . $room_id);
        if (!$is_live) {
            $this->error('直播间状态不符');
        }
