...
//            register_shutdown_function(array(&$this, 'export_dd'), $page+1);
            ...
            if($page == 1)
            {
                $content = iconv("utf-8","gbk","购物订单ID,订单来源,订单类型,订单编号,订单状态,是否允许退款,退款买方状态,退款买方配货状态,退款卖方状态,退款平台申诉,退款完成时间,退款原因,商品数量,订单总金额(含运费),商品的单价,主播抽取佣金(秀票),退款金额,运费,订单备注,收货人,收货人手机号,收货人所在地区信息,详细地址,下单时间,主播人id,购买人id,商品id,付款时间,发货时间,物流单号,物流公司,订单拥有者");
                $content = $content . "\n";
            }
