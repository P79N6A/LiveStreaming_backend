 ...
 */
    public function contribution_list()
    {
        ...
        $userlist = $GLOBALS['db']->getOne($user, true, true) ?: 'fanwe_video_prop';
        if (defined('OPEN_LIVE_PAY') && OPEN_LIVE_PAY == 1) {
            $prop = "SELECT u.id as user_id ,u.nick_name,u.head_image,SUM(v.total_diamonds) as num,SUM(v.total_ticket) as total_ticket from $userlist as v LEFT JOIN  " . DB_PREFIX . "user as u ON u.id = v.from_user_id where v.video_id=" . $video_id . " and prop_id<>12 GROUP BY v.from_user_id ORDER BY SUM(v.total_diamonds)";
            $proplist = $GLOBALS['db']->getAll($prop, true, true);
            $pay = "SELECT u.id as user_id ,u.nick_name,u.head_image,SUM(v.total_diamonds) as num,SUM(v.total_ticket) as total_ticket from " . DB_PREFIX . "live_pay_log_history as v LEFT JOIN " . DB_PREFIX . "user as u on u.id = v.from_user_id where v.video_id=" . $video_id . " GROUP BY u.nick_name ORDER BY SUM(v.total_diamonds)";
 ...
 */
    ...
    public function check_video()
    {
        ...
        if (!empty($video)) {
            ...
            if (defined('UPLOAD_OSS') && UPLOAD_OSS == 1) {
                $ret = array();
                $ret['total_count'] = 0;
                $bucket = $GLOBALS['distribution_cfg']['OSS_BUCKET_NAME'];
                $sql = "select play_url from " . DB_PREFIX . "video_history where id = " . $id;
                $url = $GLOBALS['db']->getOne($sql, true, true);
                if ($url) {
                    $pathinfo = parse_url($url);
                    $object = ltrim($pathinfo['path'], '/');
    ...
    public function delete()
    {
        ...
        if (isset($id)) {
            ...
            $url_str = '';
            foreach ($rel_data as $data) {
                if ($data['video_vid'] != '' && $data['is_del_vod'] == 0 && $data['is_delete'] == 0) {
//判断是否存在视频文件
                    if ($url_str != '') {
                        $url_str .= ',';
                    }
