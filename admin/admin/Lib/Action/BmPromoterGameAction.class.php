...
class BmPromoterGameAction extends CommonAction
{
    public function index()
    {
        ...
        $start = to_timespan("{$year}-{$month}-1 00:00:00");
        $end   = to_timespan('+1 month', $start);
    ...
    public function detail()
    {
        ...
        if ($create_time_1) {
            $where .= ' AND l.create_time > ' . to_timespan($create_time_1);
        }
        if ($create_time_2) {
            $where .= ' AND l.create_time < ' . to_timespan($create_time_2);
        }
        $pre = DB_PREFIX;
        $sql = "SELECT
                    count(1) as count
