...
class FamilyAction extends CommonAction
{
    ...
    public function view()
    {
        ...
        if ($_REQUEST['uid'] != '') {
            $where .= ' and u.id=' . intval($_REQUEST['uid']);
        }
        
        $count = $model->table($table)->where($where)->count();
        $p     = new Page($count, $listRows = 20);
        if ($count) {
            $field = 'f.*,u.nick_name,u.id,u.head_image,f.name,u.family_chieftain,u.sex,u.is_authentication';
    
            $list = $model->table($table)->where($where)->field($field)->order('u.family_chieftain desc')->limit($p->firstRow . ',' . $p->listRows)->select();
    
            foreach ($list as $key => $value) {
                $list[$key]['create_time'] = to_date($value['create_time']);
                $list[$key]['head_image'] = get_spec_image($value['head_image']);
                //身份
                if($value['family_chieftain'] == 1){
                    $list[$key]['memo'] = '族长';
                }else{
                    $list[$key]['memo'] = '成员';
                }
                ...
                //认证状态
                if($value['is_authentication'] == 1){
                    $list[$key]['authentication'] = '等待认证';
                }elseif($value['is_authentication'] == 2){
                    $list[$key]['authentication'] = '已认证';
                }elseif($value['is_authentication'] == 3){
                    $list[$key]['authentication'] = '认证未通过';
                }else{
                    $list[$key]['authentication'] = '未认证';
                }
    ...
    public function update()
    {
        ...
        $mod      = M('family');
        $family   = $mod->field('name,user_id,status,society_rank')->where("id=" . $id)->find();
        $log_info = $family['name'];
        $user_id  = $family['user_id'];
        $status   = $family['status'] == '1' ? 1 : $status;
        $society_rank = $family['society_rank'];
        $res      = $mod->save(array('memo' => $memo,'name'=>$name,'manifesto'=>$manifesto,'status' => $status, 'id' => $id, 'society_rank' => $rank));
        if (false === $res) {
            //错误提示
    ...
    public function set_sort()
    {
        ...
        $log_info = M("family")->where("id=".$id)->getField("id");
        if(!check_sort($sort))
        {
            $this->error(l("SORT_FAILED"),1);
        }
        M("family")->where("id=".$id)->setField("society_rank",$sort);
        //redis同步
