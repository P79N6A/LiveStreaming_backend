...
class EduDealAction extends CommonAction
{
    ...
    public function update()
    {
        ...
        $user_info = M("User")->where(array('id' => $deal_info['user_id'], 'is_effect' => 1))->find();
        if (empty($user_info)) {
            $this->error("发起人不存在");
        }
        ...
        $log_info = $data['name'] . "(ID：" . $data['id'] . ")";
        if (false !== $list) {
            //通知发起人项目状态
            if ($deal_info['is_effect'] == 0 && $data['is_effect'] == 1) {
                ...
                $class_id = $GLOBALS['db']->insert_id();
                $push_data = array(
                    'user_id' => $user_info['id'],
                    'nick_name' => $user_info['nick_name'],
                    'create_time' => NOW_TIME,
                    'content' => $user_info['nick_name'] . "发起了众筹「" . $deal_info['name'] . "」，去看看",
                    'ext_id' => $class_id,
                    'head_image' => get_spec_image($user_info['head_image']),
                    'status' => 0,//'推送状态(0:未推送，1：推送中；2：已推送）'
                    'push_type' => 0, //'推送状态(0:粉丝推送，1：全服推送）';
                    'type' => 10,
                );

                $GLOBALS['db']->autoExecute(DB_PREFIX . "edu_push_anchor", $push_data, 'INSERT');
            }
        ...
        if ($order_info) {
            ...
            if ($order_info['is_refund'] == 0 && $order_info['order_status'] == 1) {
                ...
                if ($GLOBALS['db']->affected_rows() > 0) {
                    ...
                    //会员秀豆增加
                    modify_account(array("diamonds" => ($order_info['pay'])), $order_info['user_id'], $msg,
                        array('type' => 20));

                    //会员账户 秀豆变更日志表
                    fanwe_require(APP_ROOT_PATH . 'mapi/lib/core/common_edu.php');
                    insert_user_diamonds_log(array('diamonds' => $order_info['pay']), $order_info['user_id'], $msg, 6,
                        $order_info['id']);
    ...
    public function save_pay_log()
    {
        ...
        if ($re) {
            ...
            //秀票变更日志表
            fanwe_require(APP_ROOT_PATH . 'mapi/lib/core/common_edu.php');
            insert_user_diamonds_log(array('ticket' => $money), $deal_info['user_id'], $log_info . "获得秀票", 7,
                $deal_info['id']);
    ...
    {
        $deal_id = $_REQUEST['deal_id'];
        $deal = M("EduDeal")->where("id=" . $deal_id . "")->find();
        if ($deal) {
            ...
            $sql2 = "SELECT v.*,watch_number+virtual_watch_number+robot_num as all_watch_number,u.nick_name FROM " . DB_PREFIX . "video_history v LEFT JOIN " . DB_PREFIX . "user u ON u.id = v.user_id  LEFT JOIN " . DB_PREFIX . "edu_video_info as ev ON ev.video_id = v.id 
				where v.group_id!='' and  v.is_delete = 0 and v.is_del_vod = 0 and ev.deal_id=" . $deal_id . "  order by v.id desc";
