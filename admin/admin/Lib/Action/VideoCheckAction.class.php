...
//name
    ...
    public function check_account()
    {
        ...
        if ($check_info) {
            ...
            $result['status'] = 0;
            $result['info'] = '13999999999账号不存在';
            print_r($result['info']);exit;
        }
    }
    public function check_video_status($video_id, $status)
    {
                ...
                if ($GLOBALS['db']->affected_rows() || 1) {
                    ...
                    if ($video['destroy_group_status'] != 1) {
                        ...
                        if ($ret['ActionStatus'] == 'OK') {
                            $sql = "update " . DB_PREFIX . "video_check set destroy_group_status = 1,group_id=id where id =" . $video['id'];
                            $GLOBALS['db']->query($sql);
                        } else {
                            $result['status'] = 1;
                    ...
                    sync_video_to_redis($video['id'], '*', false);
                    //付费记录从历史表移到原记录表中
