...
class UserCommon extends CommonAction
{
    ...
    public function index($date)
    {
        ...
        /*$parameter .= "parent_id = ".$date['user_admin_id']."&";
        $parameter .= "user_iden_id = ".$date['user_iden_id']."&";
        $sql_w .= "parent_id=" .$date['user_admin_id']. " and   user_iden_id = ".$date['user_iden_id']." and ";*/

        //if($date['id'] != "" || $date['mobile'] != "" || $date['nick_name'] != "" || $date['create_time_1'] != "" || $date['btn_create_time_1'] != ""){
            $parameter .= "parent_id = ".$date['user_admin_id']."&";
            $sql_w .= "parent_id=" .$date['user_admin_id']. "  and ";
        ...
        if (intval($date['id']) == 0 || $date['mobile'] != "" || $date['nick_name'] != "" || $date['create_time_1'] != "" || $date['btn_create_time_1'] != "" ) {
            $sql_str .= " and " . $sql_w . " 1=1 ";
            $count_sql .= " and " . $sql_w . " 1=1) ";
        } else {
            $sql_str .= " and " . $sql_w . " 1=1 union SELECT *," .
                " ticket-refund_ticket as useable_ticket ,floor(score + online_time*" . $ote . ") as u_score" .
                " FROM " . DB_PREFIX . "user WHERE id=" . $date['id'];
            $count_sql .= " and " . $sql_w . " 1=1 ) or (id=" . $date['id'] . ")";
        }
       ...
       // dump($sql_str);die;
        ...
        //处理数组，去除不是当前指定等级的会员
        foreach ($voList as $k=>$v){
            if($v['user_iden_id'] != $date['user_iden_id']){
                unset($voList[$k]);
            }
        }
    ...
    public function index_user_data($date)
    {
        ...
        $voList = M("user")->where("parent_id = ".$date['user_admin_id'])->order("id desc")->select();
        ...
        $whole = "1=1 and user_iden_id = {$date['user_iden_id']} ";
    ...
    public function user_data($voList,$user_iden_id,$voList_user){
        ...
        foreach ($voList_user as $k2=>$v2){
            if($v2['user_iden_id'] == $user_iden_id){
                $voList_uses2[] = $v2;
            }
        }
    ...
    public function edit($date)
    {
        ...
        //当前会员id
        $admin_user_id = $_SESSION['user_admin_session']['id'];
        $admin_user_data = M("user")->field("user_iden_id")->where("id = {$admin_user_id}")->find();
        ...
        $vo['admin_user_data'] = $admin_user_data['user_iden_id'];
...
//        //②用户认证状态为未通过(3)状态 AND 需要身份验证, 则显示身份验证
    ...
    public function user_update(){
        ...
        $re = M("user")->where("id = {$id}")->save(array("user_status" => $user_status,"parent_id"=>$parent_id));
