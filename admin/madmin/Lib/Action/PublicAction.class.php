...
class PublicAction extends BaseAction
{
    ...
    public function do_login()
    {
        ...
        //判断用户是否存在
        $user_admin = M("user")->where("mobile = '{$adm_name}'")->find();

        //只有有分销账号能登陆分销管理后台
        if($user_admin['user_iden_id'] == 0){
            $this->error('无权限', $ajax);
        }

        if($user_admin){
            //如果用户存在判断密码
            if($user_admin['user_pwd'] == md5($adm_password)){

                //如果状态是未审核不能登陆
                if($user_admin['user_status'] == 0){
                    $this->error('等待审核', $ajax);
                }else{
                    $_SESSION["user_admin_session"] = $user_admin;
    ...
    public function do_loginout()
    {
        ...
        if ($user_admin_session == "") {
            //已登录
            $this->redirect(u("Public/login"));
        } else {
            $_SESSION['user_admin_session'] = "";
            $this->success(L("LOGINOUT_SUCCESS"));
        }
