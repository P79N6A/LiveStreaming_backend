...
var canCheck = true, code_timeer = null, canSendSmsVerify = true, isCheckAccountIp = false;
var checkLoginAction = {
	checkAccountIp: function(callback){
		...
		$.ajax({
            ...
            success: function(obj){
                if(obj.status==1){
                   isCheckAccountIp = true;
                   (callback == 'function') && callback.call(this, isCheckAccountIp);
                }
                else{
                	isCheckAccountIp = false;
                	(callback == 'function') && callback.call(this, isCheckAccountIp);
                }
            }
        });
...
$(document).ready(function(){
	if(document.getElementsByName("is_check_account")[0].value){
		checkLoginAction.checkAccountIp(function(data){
			if(data){
				canCheck = false;
				document.querySelectorAll(".tr_smsVerify")[0].style.display = 'none';
				document.querySelectorAll(".tr_smsVerify")[1].style.display = 'none';
			}
		});
	}
	else{
