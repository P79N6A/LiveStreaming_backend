...
$(document).ready(function() {
    ...
    $('.J_autoUserName').live('focus', function(event) {
        ...
        obj.autocomplete(ROOT + "?m=Public&a=autoloaduser", {
            ...
            extraParams: {
                user_type: function() {
                    return (obj.attr("user_type") == undefined ? 0 : obj.attr("user_type"))
                }
            },
            parse: function(data) {
...
// //车行定制-修改回播授权
// function set_video_authorization(id,domobj){
//
//     $.ajax({
//         url: ROOT+"?"+VAR_MODULE+"="+MODULE_NAME+"&"+VAR_ACTION+"=set_video_authorization&id="+id,
//         data: "ajax=1",
//         dataType: "json",
//         success: function(obj){
//
        ...
        editor.loadPlugin('insertfile', function() {
            editor.plugin.fileDialog({
                clickFn: function(url, title) {
                    console.log(dom);
                    dom.find("#kefile_" + node.attr("rel")).val(url);
...
(function() {
    ...
    $(".opration").live('click', function(e) {
        var window_height = $(window).height(),
            opration_offset_top = $(this).offset().top;
...
function open_avatar_view(w, h, obj) {
    attr_id = obj;
    $.weeboxs.open(APP_ROOT + "/index.php?ctl=ajax&act=upload_img&w=" + w + "&h=" + h + "&dst=" + attr_id, {
        boxid: 'avatar-box',
        contentType: 'ajax',
        showButton: false,
        showCancel: false,
