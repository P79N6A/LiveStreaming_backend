...
function api_ajax_return($data,$is_debug=false)
{
	...
	if($_REQUEST['post_type']!='json'){
		...
		if($data['user_login_status'] === 0){
			if (isset($_REQUEST['tmpl_pre_dir'])) {
				$tmpl_pre_dir = strim($_REQUEST['tmpl_pre_dir']);
				$tmpl_dir = "{$tmpl_pre_dir}/login-nav.html";
			} else {
				$tmpl_dir = "login-login.html";
			}
		}else {
			$tmpl_dir = $GLOBALS['class_name'] . '-' . $GLOBALS['act'] . '.html';
			if (isset($_REQUEST['tmpl_pre_dir'])) {
				$tmpl_pre_dir = strim($_REQUEST['tmpl_pre_dir']);
				$tmpl_dir = $tmpl_pre_dir . '/' . $tmpl_dir;
			}
		}
