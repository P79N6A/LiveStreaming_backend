...
var vm = avalon.define({
    ...
    login: function() {
       if(! this.check()){
        ...
        query.pc = 1;
        $.ajax({
            url:APP_ROOT+"/mapi/index.php?ctl=login&act=do_login&itype=app",
            data:query,
            type:"POST",
            dataType:"json",
            success:function(result){
                if(result.status == 1){
                    $.weeboxs.close("login_box");
                    if(result.error){
                        if(result.is_lack == 1){
                            $.showErr("您的昵称或者头像还未完善，请前往修改", function(){
                        ...
                        if(result.is_lack == 1){
                            $.showSuccess("您的昵称或者头像还未完善，请前往修改", function(){
                                location.href = APP_ROOT+"/index.php?ctl=user&act=userinfo";
                            });
                        }
                        else{
