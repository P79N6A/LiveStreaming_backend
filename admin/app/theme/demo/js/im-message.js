...
function convertCustomMsgToHtml(content) {
    ...
    switch (msg.type) {
        ...
        case 1: // 收到发送礼物消息
            ...
            var gif = msg.pc_gif || icon;
            text = msg.desc + '&nbsp;<img class="icon" style="height:2em;" src="' + icon + '" />';
            if (msg.is_much == 1 && msg.plus_num > 1) {
                var domId = msg.sender.user_id + "_" + msg.prop_id;
                giftlist(domId, "/app/theme/demo/images/gift/gift_bj.png", gif, nick_name, "", msg.plus_num);
            } else {
                var liwu = document.getElementById("liwu");
                if (liwu) {
                    liwu.addLiWuTip(JSON.stringify({
                        "duration": 15,
                        "icon": {
                            "src": gif,
                            "width": 80,
                        ...
                        "text": {
                            "bgcolor": "0x0099FF",
                            "content": "<font color=\'#000000\' size=\'15\' face=\'微软雅黑\'>" + nick_name + ":" + msg.desc + "<b></b></font>",
                            "padding": 90,
                            "top": 44,
                            "left": 0,
                            "height": "36"
        ...
        case 20: // 私聊文字消息
            ...
            avChatRoomId = typeof avChatRoomId === 'undefined' ? '' : avChatRoomId;
            if (msg.group_id && avChatRoomId && avChatRoomId != msg.group_id) {
                return;
            }
            break;
        case 21: // 私聊语音消息
    ...
    return {"type": msg.type, "user_level": user_level, "nick_name": nick_name, "text": text};
}
...
var im_message = {
    ...
    sendTextMsg: function (to_uid, text, type, is_c2c) {
        ...
        webim.sendMsg(msg, function (resp) {
            ...
            error_str = error_str.SrcErrorInfo.replace(/you has been shut up/, "您已被禁言");
