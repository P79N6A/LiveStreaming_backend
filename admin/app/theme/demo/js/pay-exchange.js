    ...
    submit: function(){
        ...
        if(vm.data.rule_id <= 0&& vm.data.other_diamonds <= 0){
            ...
            handleAjax.handle(APP_ROOT+"/mapi/index.php?ctl=pay&act=do_exchange",vm.data).done(function(msg){
                ...
                $.showErr(err,function(){location.href = APP_ROOT+"/index.php?ctl=user&act=userinfo";});
            });
        }
    }
});
...
$("input[name='rule_id']").bind('keypress',function(event){
    if(event.keyCode == "13") vm.submit();
});
$("input[name='ticket']").bind('keypress',function(event){
    if(event.keyCode == "13") vm.submit();
});