...
<script type="text/javascript" src="' . SITE_DOMAIN . '/admin/Tpl/default/Common/js/jquery.js"></script>
<div align="center" style="padding-top: 50px;">
<form action="' . SITE_DOMAIN . '/syn/index.php?ctl=syn&act=do_action2" method="get" id="form_type">
    清除信息：<select name="type" id="type" onchange="change_type();">
    <option value="0">选择数据的操作方式</option>
    <option value="3">修改等级为1（积分清零）</option>
    <option value="4">用户的秀豆、消费秀豆、秀票、可用秀票清零</option>
...
<script type="text/javascript">
    ...
    public function syn_user_level($json = 0, $user_id = 0)
    {
        if (IS_DEBUG) {
            ...
            $GLOBALS['db']->query("update " . DB_PREFIX . "user set score=0,online_time=0,user_level=1,anchor_level=1 where is_robot = 0 " . $sql);
            $user_data = $GLOBALS['db']->getAll("select * from " . DB_PREFIX . "user where is_robot = 0 " . $sql, true, true);
            if (count($user_data) > 0) {
                fanwe_require(APP_ROOT_PATH . 'mapi/lib/redis/BaseRedisService.php');
                fanwe_require(APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php');
                $user_redis = new UserRedisService();
