...
(function($) {
    $.fn.picLazyLoad = function(settings) {
        ...
        lazyLoadPic();
        $(".content").on('scroll', function() { _winScrollTop = $(window).scrollTop();
            lazyLoadPic(); });
            ...
            $this.each(function() {
                ...
                if ($self.is('img')) {
                    if ($self.attr('data-original')) {
                        ...
                        if ((_offsetTop - settings.threshold) <= (_winHeight + _winScrollTop)) { $self.addClass('animated fadeIn').attr('src', $self.attr('data-original'));
                            $self.removeAttr('data-original'); } } } else {
                    if ($self.attr('data-original')) {
                        if ($self.css('background-image') == 'none') { $self.css('background-image', 'url(' + settings.placeholder + ')'); }
                        var _offsetTop = $self.offset().top;
                        if ((_offsetTop - settings.threshold) <= (_winHeight + _winScrollTop)) { $self.css('background-image', 'url(' + $self.attr('data-original') + ')');
