...
class appCModule extends baseModule
{
    ...
    public function init()
    {
        ...
        //每次登录时升级提示
        $root['open_upgrade_prompt'] = OPEN_UPGRADE_PROMPT;
        ...
        if ((defined('OPEN_LIVE_PAY') && OPEN_LIVE_PAY == 1) && intval($ios_check_version) == 0) {
            $live_pay = $GLOBALS['db']->getAll("SELECT id FROM " . DB_PREFIX . "plugin WHERE is_effect=1 and type =1");
            if ($live_pay == null) {
            } else {
            ...
            if ((defined('LIVE_PAY_TIME') && LIVE_PAY_TIME == 1)) {
                if ($live_pay[0]['id'] == 3 || $live_pay[1]['id'] == 3) {
            ...
            if ((defined('LIVE_PAY_SCENE') && LIVE_PAY_SCENE == 1)) {
                if ($live_pay[0]['id'] == 4 || $live_pay[1]['id'] == 4) {
            ...
            if (defined('OPEN_PAI_MODULE') && OPEN_PAI_MODULE == 1) {
                if ($plugin['class_name'] == 'pai') {
                    if ($video['pai_id'] != 0) {
                        $pai_goods = $GLOBALS['db']->getRow("SELECT create_time,pai_time FROM " . DB_PREFIX . "pai_goods WHERE id=" . $video['pai_id'] . " ");
                        if ($pai_goods) {
                            if (NOW_TIME < $pai_goods['create_time'] + $pai_goods['pai_time'] * 3600) {
                                $is_enable = 0;
                                $error = '竞拍正在使用中...';
                            }
                        }
