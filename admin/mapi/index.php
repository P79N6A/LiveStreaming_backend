...
//会员自动登录及输出
    $cookie_uid = es_cookie::get("user_id") ? es_cookie::get("user_id") : '';
    $cookie_upwd = es_cookie::get("user_pwd") ? es_cookie::get("user_pwd") : '';
    $promoter_id = es_cookie::get("promoter_id") ? es_cookie::get("promoter_id") : '';

    if ($cookie_uid != '' && $cookie_upwd != '' && !es_session::get("user_info")) {
        //fanwe_require(APP_ROOT_PATH."system/libs/user.php");
        fanwe_require(APP_ROOT_PATH . "mapi/lib/base.action.php");
        fanwe_require(APP_ROOT_PATH . "mapi/bm_index/login.action.php");
        $login = new loginCModule();
        $login->auto_do_login_user($cookie_uid, $cookie_upwd, $promoter_id);
    }
...
//  es_session::set("user_info",$user_info);
    ...
    global $promoter_info;
    $promoter_info = es_session::get('promoter_info');
//$role_list=json_decode($promoter_info['role_list'],true);
    $role_list = unserialize($promoter_info['role_list']);
    ...
    if ($lib == 'lib') {
        ...
        if (file_exists(APP_ROOT_PATH . "mapi/" . $lib . "/base.action.php")) {
            @fanwe_require(APP_ROOT_PATH . "mapi/" . $lib . "/base.action.php");
        }
        if (file_exists(APP_ROOT_PATH . "mapi/" . $lib . "/" . $class . ".action.php")) {
            @fanwe_require(APP_ROOT_PATH . "mapi/" . $lib . "/" . $class . ".action.php");
