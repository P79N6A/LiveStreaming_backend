...
class UserCommon extends CommonAction{
	...
	public function index($date)
	{
        ...
        {
            $parameter.= "member_type=" . intval($date['member_type']). "&";
            $sql_w .= "member_type=".intval($date['member_type'])." and ";
        }
        if($_REQUEST['is_agent']!='')
        {
            $parameter.= "is_agent=" . intval($date['is_agent']). "&";
            $sql_w .= "is_agent=".intval($date['is_agent'])." and ";
        }
        if($_REQUEST['is_canal']!='')
        {
            $parameter.= "is_canal=" . intval($date['is_canal']). "&";
            $sql_w .= "is_canal=".intval($date['is_canal'])." and ";
        }
        if($_REQUEST['is_star_share']!='')
        {
            $parameter.= "is_star_share=" . intval($date['is_star_share']). "&";
            $sql_w .= "is_star_share=".intval($date['is_star_share'])." and ";
        }
		...
		$voList = $this->_Sql_list($model, $sql_str, "&".$parameter,'id');
		foreach($voList as $k=>$v){
			$voList[$k]['head_image'] = get_spec_image($v['head_image'],40,40);
		}
    ...
    public function insert(){
        ...
        $res = save_user_wpk($_REQUEST,'INSERT',$update_status=1);
        if($res['status']==0)
        {
	...
	public function edit($date) {
        ...
        $vo['user_level'] = $vo['user_level']>0?$vo['user_level']:1;
		$this->assign ( 'vo', $vo );
		$region_pid = 0;
		$region_lv2 = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."region_conf where region_level = 2 order by py asc");  //二级地址
		...
		//会员等级信息
		$user_level = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_level where `level`>=".$vo['user_level']." order by level ASC");
		$this->assign("user_level",$user_level);
	...
	public function update() {
		...
		$user_info = array_merge($user_info,$data);
  		$res = save_user_wpk($user_info,'UPDATE',$update_status=0);
   ...
   /* public function del_contribution_list($data)
	...
	public function modify_account($data)
	{
		...
		$data_arr['money'] = floatval($data['money']);

		$msg = trim($data['msg'])==''?l("ADMIN_MODIFY_ACCOUNT"):trim($data['msg']);
		
		modify_account($data_arr,$user_id,$msg);
        if ($data_arr['money'] >= 0)
		    save_log("用户".$user_id."余额增加".$data_arr['money']."成功",1);
        elseif ($data_arr['money'] < 0){
            save_log("用户".$user_id."余额减少".(-$data_arr['money'])."成功",1);
        }
		return true;
	}
    ...
    public function prop($data)
    {
        ...
        if(trim($data['create_time_1'])!=''&&date('Ym',$create_time_1)!=date('Ym',NOW_TIME)){
            $sql_str = "SELECT l.id,l.create_ym,l.to_user_id, l.create_time,l.prop_id,l.prop_name,l.from_user_id,l.create_date,l.num,l.total_ticket,u.nick_name
                         FROM   ".DB_PREFIX."video_prop_".date('Ym',$create_time_1)." as l
        ...
        $volist = $this->_Sql_list($model,$sql_str,'&'.$parameter,1);
        foreach($volist as $k=>$v){
