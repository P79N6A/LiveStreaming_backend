...
class rankCModule  extends baseModule
{

    /**
     * 贡献排行榜 from_user_id 送   无关注  贡献（消费）榜是按：送出的秀豆 排序  ===》total_diamonds
     * 分为 ： 日榜（缓存时间最低：1800s）、周榜（缓存时间最低：3600s）
	...
	public function contribution(){
		...
		if(in_array($rank_name,$rank_names)){
			...
			$rank_cache_default =array("rank_day"=>1800,"rank_week"=>3600);
			...
			$list = load_auto_cache("rank_car_contribution",$param);
    ...
    /**
     * 收入排行榜 to_user_id 收  无关注  收入榜：按收到的秀票 排序 ===> sum(total_ticket)   where is_red_envelope = 0
	...
	public function consumption(){
		...
		if(in_array($rank_name,$rank_names)){
			...
			$param = array('rank_name'=>$rank_name,'table'=>$table,'page'=>$page,'page_size'=>$page_size,'cache_time'=>$rank_cache_time);
			$list = load_auto_cache("rank_car_consumption",$param);
			fanwe_require(APP_ROOT_PATH.'mapi/lib/redis/UserFollwRedisService.php');
			$user_redis = new UserFollwRedisService($user_id);
			$keys = $user_redis->following();
