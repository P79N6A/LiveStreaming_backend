...
class loginCModule  extends baseModule
{
	...
	public function do_login_promoter($user_id_or_mobile,$verify_code,$p_user_id){
		...
		if(!$user)
		{
			...
			es_cookie::set("promoter_id",intval($promoter['id']),3600*24*30);
				
			//设置session
			es_session::set("user_info",$user);
			$GLOBALS['user_info'] = $user;		
			$promoter['role_list']=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."bm_role where (id='".$promoter['bm_role_id']."'  ) ");
			es_session::set("promoter_info",$promoter);
			$GLOBALS['promoter_info'] = $promoter;
		}
	...
	public function do_login_promoter_2($user_id_or_mobile,$password,$p_user_id){
		...
		if($user_id_or_mobile!=''){
			...
			$result['info'] = "请输入手机号或者账户名";
			return $result;
		}
	
		$promoter = $GLOBALS['db']->getRow("select * from  " . DB_PREFIX . "bm_promoter where (mobile='".$user_id_or_mobile."' or login_name = '".$user_id_or_mobile."' ) and is_effect=1 and status=1", true, true);
		if (!$promoter) {
			$result['info'] = "帐户无登陆权限，请联系管理员";
			return $result;
		}
	...
	public function auto_do_login_user($user_id,$user_md5_pwd,$promoter_id){
		...
		if($user_data)
		{
			...
			{
				$bm_promoter=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."bm_promoter where id='".$promoter_id."' and is_effect = 1 and status=1 ");
				if (!$bm_promoter) {
					$bm_promoter_type=0;
				...
				$promoter['role_list']=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."bm_role where (id='".$promoter['bm_role_id']."'  ) ");
				es_session::set("promoter_info",$bm_promoter);
				$GLOBALS['promoter_info'] = $bm_promoter;
	...
	public function logout(){
		...
		es_session::delete("bm_promoter");
		es_cookie::set("promoter_id",'',0);
		es_session::delete("act_list");
