...
class family1_userModule extends baseModule
{
    //家族成员列表
    public function user_list()
    {
        $root = array();
        if (!$GLOBALS['user_info']&&0) {
...
//                $root['rs_count'] = $count;//家族成员总数
                ...
                //主播人数
                $user = $GLOBALS['db']->getAll("SELECT id as user_id,nick_name,sex,v_type,v_icon,head_image,signature,user_level,family_chieftain,is_authentication FROM " . DB_PREFIX . "user WHERE family_id=" . $family_id . " and is_authentication = 2 ORDER BY family_chieftain desc limit " . $limit, true, true);
                $count=strval(count($user));
                $root['rs_count'] = $count;
                //粉丝人数
                $follower = $GLOBALS['db']->getAll("SELECT id as user_id,nick_name,sex,v_type,v_icon,head_image,signature,user_level,family_chieftain,is_authentication FROM " . DB_PREFIX . "user WHERE family_id=" . $family_id . " and is_authentication <> 2 ORDER BY family_chieftain desc limit " . $limit, true, true);
                $root['follower_count'] = strval(count($follower));
                foreach ($user as $k => $v) {
...
//                $root['rs_count'] = $count;//家族成员总数
                ...
                //主播人数
                $user = $GLOBALS['db']->getAll("SELECT id as user_id,nick_name,sex,v_type,v_icon,head_image,signature,user_level,family_chieftain,is_authentication FROM " . DB_PREFIX . "user WHERE family_id=" . $family_id . " and is_authentication = 2 ORDER BY family_chieftain desc limit " . $limit, true, true);
                $root['rs_count'] = strval(count($user));
                //粉丝人数
                $follower = $GLOBALS['db']->getAll("SELECT id as user_id,nick_name,sex,v_type,v_icon,head_image,signature,user_level,family_chieftain,is_authentication FROM " . DB_PREFIX . "user WHERE family_id=" . $family_id . " and is_authentication <> 2 ORDER BY family_chieftain desc limit " . $limit, true, true);
                $count=strval(count($follower));
                $root['follower_count'] =$count;
                foreach ($follower as $k => $v) {
                    $follower[$k]['head_image'] = get_spec_image($v['head_image']);
                }
                $root['list'] = $follower;//家族成员信息
    ...
    public function r_user_list()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if ($family_id > 0 && $family_chieftain == 1 && $family) {
                ...
                $root['status'] = 1;
                //申请人数
                $count =$apply_count = $GLOBALS['db']->getOne("SELECT COUNT(id) as apply_count FROM " . DB_PREFIX . "family_join WHERE family_id=" . $family_id . " and status=0", true, true);
                $root['apply_count'] = $apply_count;
                //主播人数
                $user = $GLOBALS['db']->getAll("SELECT id as user_id,nick_name,sex,v_type,v_icon,head_image,signature,user_level,family_chieftain,is_authentication FROM " . DB_PREFIX . "user WHERE family_id=" . $family_id . " and is_authentication = 2 ORDER BY family_chieftain desc limit " . $limit, true, true);
                $root['rs_count'] = strval(count($user));
