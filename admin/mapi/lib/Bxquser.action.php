...
class userModule extends baseModule
{
    ...
    public function forbid_send_msg()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $user_second = intval($_REQUEST['second']); //禁言时间，单位为秒; 为0时表示取消禁言
            if ($user_second) {
                if ($user_second == 600) {
                    $user_unit = '10分钟';
                } elseif ($user_second == 3600) {
                    $user_unit = '1小时';
                } elseif ($user_second == 43200) {
                    $user_unit = '12小时';
                } elseif ($user_second == 86400) {
                    $user_unit = '1天';
                } elseif ($user_second > 86400) {
                    $user_unit = '永久';
                }
            ...
            if ($allow) {
                ...
                if ($ret['ActionStatus'] == 'OK' || $user['is_robot'] == 1) {
                    ...
                    if ($second > 0) {
                        ...
                        $msg = $nick_name . " 被禁言" . $user_unit; //.print_r($ret,1).';user_id:'.$user_id.";second:".$second.";group_id:".$group_id;
                        $root['is_forbid'] = 1;
