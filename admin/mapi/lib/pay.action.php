...
class payModule extends baseModule
{
    ...
    public function recharge()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $first_count = $GLOBALS['db']->getOne('SELECT COUNT(id) FROM `' . DB_PREFIX . 'payment_notice` WHERE user_id = ' . $user_id . ' AND is_paid = 1');
            /*
            ...
            if (defined('RECHARGE_CODE') && RECHARGE_CODE == 1) {
                $code_rule_list = load_auto_cache("coderule_list");
            }
            ...
            foreach ($pay_list as $k => $v) {
                ...
                if (defined('RECHARGE_CODE') && RECHARGE_CODE == 1) {
                    if ($v['class_name'] == 'Codeapp') {
                        if (is_array($code_rule_list)) {
                            $list = $code_rule_list;
                        }
                    }
                }
                ...
                foreach ($list as &$_value) {
                    if (intval($first_count) == 0 && $_value['is_first_charge'] == 1) {
                        $_value['is_payed'] = 0;
                    } else {
                        $_value['is_payed'] = 1;
                    }
            ...
            foreach ($rule_list as &$_value_) {
                if (intval($first_count) == 0 && $_value_['is_first_charge'] == 1) {
                    $_value_['is_payed'] = 0;
                } else {
                    $_value_['is_payed'] = 1;
                }
            ...
            $root['rate'] = $diamonds_rate; //充值金额与秀豆的换算比率如：充值1元，可以获得10个秀豆
        }
    ...
    public function pay()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if ($pay_id == 0) {
                ...
                if ($rule_id > 0) {
                    $sql = "select money,name,iap_money,product_id,(diamonds+gift_diamonds) as diamonds,is_first_charge,first_gift_diamonds from " . DB_PREFIX . "recharge_rule where is_effect = 1 and is_delete = 0 and id =" . $rule_id;
                    ...
                    if (intval($first_count) == 0 && $rule['is_first_charge'] == 1) {
                        $diamonds = bcadd($rule['diamonds'], $rule['first_gift_diamonds']);
                    } else {
