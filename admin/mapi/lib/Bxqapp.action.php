...
class appModule extends baseModule
{
	...
	public function init()
	{
		...
		$root['monitor_second'] = 5;//主播心跳监听，每5秒监听一次;监听数据：时间点，秀票数，房间人数
        ...
        */
		...
		//手机端配置,腾讯云 账号信息
		$root['sdkappid'] = trim($m_config['SdkAppId']);
		$root['accountType'] = trim($m_config['accountType']);
 ...
 * */
	public function plugin_init()
    {
        ...
        $open_plugin = $GLOBALS['db']->getRow("SELECT open_pay,open_game,open_auction FROM  ".DB_PREFIX."user WHERE id=$user_id");
        foreach ($plugin as $key => $value) {
            $plugin[$key]['is_active'] = 0;
            switch ($value['class_name']) {
                case 'game':
                    if (defined('OPEN_GAME_MODULE') && OPEN_GAME_MODULE == 1 && $open_plugin['open_game']==0) {
                        if ($value['type'] == 2) {
                            $game_id   = 0;
                            $is_enable = 1;
                            if ($video['id']) {
                ...
                case 'pai':
                    if (defined('OPEN_PAI_MODULE') && OPEN_PAI_MODULE == 1&&$open_plugin['open_auction']==0) {
                        if ($value['type'] == 1) {
                            if ($video['pai_id'] != 0) {
                ...
                case 'live_pay_scene':
                default:
                    if (defined('OPEN_LIVE_PAY') && OPEN_LIVE_PAY == 1 &&$open_plugin['open_pay']==0) {
