...
class DeZhou extends Poker
{
     ...
     * @var array
     */
    protected $type = array('thds' => 0, 'ths' => 1, 't4' => 2, 'hl' => 3, 'th' => 4, 'sz' => 5, 't3' => 6, 'd2' => 7, 'd1' => 8, 'gp' => 9);
    public $gp = array();
    ...
    public function checkCards($cards)
    {
        if (sizeof($cards) == 5) {
            ...
            $figure_flag = false;
            $figures = array();
            $colors = array();foreach ($cards as $key => $value) {
                $colors[$key] = $color = $this->colors[$value[0]];
                $figures[$key] = $figure = $this->figures[$value[1]];if (!$key) {
                    $color_flag = $color;
                } else {
                    if ($color_flag != $color) {
                        $th = false;
