 ...
 */
    ...
    public function inc_score($user_id, $val)
    {
        ...
        //更新等级
        $user_data = $this->getRow_db($user_id, array('id', 'score', 'online_time', 'user_level', 'ticket', 'refund_ticket', 'anchor_level'));
    ...
    /*
     * 同一时间内 ，同一用户 秀豆数 只允许操作一次，并发时候 会延时10ms
     * @return 若减少的秀豆数，少于当前秀豆数，则返回false;否则返回改变后的数量
     */
    ...
    public function online_set_add($id)
    {
        return $this->redis->sAdd($this->user_db . 'online_users', $id);
    }
    public function online_set_rm($id)
    {
        return $this->redis->srem($this->user_db . 'online_users', $id);
    }
    public function online_users_num()
    {
        return $this->redis->sCard($this->user_db . 'online_users');
    }
}
