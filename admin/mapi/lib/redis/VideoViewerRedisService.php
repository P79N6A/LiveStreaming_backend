...
class VideoViewerRedisService extends BaseRedisService
{
    ...
    public function member_join($post)
    {
        ...
        foreach ($post['NewMemberList'] as $k => $v) {
            ...
            //设置用户最后观看的房间id
            $this->redis->hSet($this->user_db . 'user_last_views_room', $user_id, $video_id);
    ...
    public function member_exit($post)
    {
        ...
        foreach ($post['ExitMemberList'] as $k => $v) {
            ...
            if ($this->redis->zRem($this->video_viewer_level_db . $video_id, $user_id) > 0) {
                //清除用户最后观看的房间信息
                $this->redis->hDel($this->user_db . 'user_last_views_room', $user_id);
                /*
                //实际观众看人数-1
                $watch_number = $this->redis->hIncrBy($this->video_db.$video_id,'watch_number',-1);
    ...
    public function get_viewer_list2($video_id, $page = 0, $page_size = 200)
    {
        ...
        $video = $this->redis->hMGet($this->video_db . $video_id, array('user_id'));
        ...
        if (is_array($user_list_array)) {
            foreach ($user_list_array as $k => $v) {
                ...
                if ($v) {
                    ...
                    $user2['sex'] = $user['sex'];
                    $user2['user_ticket'] = (int) load_auto_cache("user_prop_anchor_num", array('from_user_id' => $k, 'to_user_id' => $video['user_id']));
                    $user_list[] = $user2;
                }
     ...
     * 会员掉线离开
     */
    public function member_drop_line_exit($user_id)
    {
        $video_id = $this->redis->hGet($this->user_db . 'user_last_views_room', $user_id);
        if (empty($video_id)) {
            return false;
        }
        $video = $this->redis->hMGet($this->video_db . $video_id, array('room_type', 'virtual_number', 'virtual_watch_number'));
    ...
    /**
     * [memberid_to_videoid 获取会员进入的直播间信息]
     */
    public function memberid_to_videoid($user_id)
    {
        return (int) $this->redis->hGet($this->user_db . 'user_last_views_room', $user_id);
    }
}
