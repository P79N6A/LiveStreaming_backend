...
class VideoContributionRedisService extends BaseRedisService
{
    ...
    public $user_contribution; //:podcast_id   zset 有序数据,user_id:贡献数
    public $video_contribution; //：video_id  zset 有序数据,user_id:贡献数

    public $video_vote_number_db; //   zset video_id:映票数(vote_number)
...
//    public function test_add_redis($user_list){
     ...
     */
    public function insert_db($user_id, $podcast_id, $video_id = 0, $num, $total_society_ticket = 0)
    {
        $data = array();
        if ($video_id) {
            $data = $this->redis->hMGet($this->video_db . $video_id, array('province', 'sex', 'title', 'room_type', 'live_in', 'in_livepk'));
        }
        $pipe = $this->redis->multi();
        //主播获取ticket，发送人 获取积分
        ...
        if ($video_id) {
            ...
            $pipe->hIncrBy($this->video_db . $video_id, 'society_ticket', $total_society_ticket);
    ...
    public function get_video_contribute($video_id, $page, $page_size = 20, $is_only_list = false)
    {
        ...
        //$user_id= $GLOBALS['db']->getOne('select user_id from '.DB_PREFIX.'video where id='.$video_id);
    ...
    public function get_video_contribute_week($video_id, $page, $page_size = 20, $is_only_list = false)
    {
        $root = array();
        if ($page == 0) {
            $page = 1;
        ...
        $param = array('user_id' => $user_id, 'table' => $table, 'page' => $page, 'page_size' => $page_size, 'cache_time' => $rank_cache_time);
        $user_list = load_auto_cache("video_contribute_week", $param);
