...
class BaseRedisService
{
    ...
    public $prefix;
    public $auto_id_db; //hash数据 存储 user_id  room_id  gift_id

    public $video_db; //:video_id  hash数据
    public $video_group_db; // hMSet 有 group:id:video_id

    public $user_db; //:user_id  hash数据
    public $user_hash_db; //所有会员数据 user_id hash数据 存储在线数据
    public $user_robot_db; //user_id  set数据 ,机器人的集合

    public $video_viewer_level_db; //:video_id  zset 房间观众列表,user_id:会员级别

    //======观众权重=========
    public $gz_level_weight = 1000; //等级权重
    public $gz_rz_weight = 500; //认证权重
    public $gz_real_weight = 2000000; //真人权重

    public $timeout = 120; // 锁时间
    ...
    public function asyncWork($lock_name, callable $func, callable $func2)
    {
        register_shutdown_function(function () use (&$lock_name) {
            $this->redis->del($this->lock_db . $lock_name);
        });
        $result = null;
        if ($this->redis->setnx($this->lock_db . $lock_name, 1)) {
            try {
            ...
            try {
                ...
                if ($this->redis->ttl($this->lock_db . $lock_name) == -1) {
                    $this->redis->expire($this->lock_db . $lock_name, $this->timeout);
                }
            }
        }
        return $result;
