...
class loginModule extends baseModule
{
    ...
    public function do_login()
    {
        ...
        if (trim($_REQUEST['mobile']) == '13888888888' || trim($_REQUEST['mobile']) == '13999999999') {
            ...
            if ($result['status']) {
                ...
                } elseif ($GLOBALS['db']->getOne("SELECT login_ip FROM " . DB_PREFIX . "user WHERE login_ip = '" . get_client_ip() . "' and ban_type = 1 and is_ban =1")) {
                ...
                if (is_array($_REQUEST)) {
                    $log_date['request'] = json_encode($_REQUEST);
                }
    ...
    public function do_update()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $system_head_image = $m_config['head_image'];
...
//修改头像
            ...
            log_file($user_data, 'do_update');
            $res = update_mobile_user($user_data, 'UPDATE');
            log_file($res, 'do_update');
            if ($res['status'] == 1) {
                //更新session
    ...
    public function wx_login()
    {
        ...
        if (($_REQUEST['openid'] != "" && $_REQUEST['access_token'] != "") || $_REQUEST['code'] != "") {
            ...
            if ($wx_info && intval($root['status']) != 0) {
                ...
                $log_date = array();
                if (is_array($_REQUEST)) {
                    $log_date['request'] = json_encode($_REQUEST, true);
                }
    ...
    public function facebook_login()
    {
        $root = array('status' => 1, 'error' => '');
        define('DEBUG_FB', 1);
        fanwe_require(APP_ROOT_PATH . "system/libs/facebook.php");
        $m_config = load_auto_cache("m_config"); //初始化手机端配置
        $ajax = 1;
        //获取微信配置信息
        if (DEBUG_FB) {
            log_result('--脸书登录回调调试开始--');
            log_result('-获取脸书配置信息-');
            log_result('-fb_appid-');
            log_result($m_config['facebook_appid']);
            log_result('-facebook_secrit-');
            log_result($m_config['facebook_secrit']);
        }
        //登录限制（相同IP都限制）
        if ($GLOBALS['db']->getOne("SELECT login_ip FROM " . DB_PREFIX . "user WHERE login_ip = '" . get_client_ip() . "' and is_effect !=1")) {
            ajax_return(array("status" => 0, "error" => '当前IP已被封停'));
        ...
        if ($m_config['facebook_appid'] == '' || $m_config['facebook_secrit'] == '') {
            $root['status'] = 0;
            $root['error'] = "facebook_appid或facebook_secrit不存在";
            ajax_return($root);
        } else {
            $facebook_appid = strim($m_config['facebook_appid']);
            $facebook_secrit = strim($m_config['facebook_secrit']);
        }

        $jump_url = SITE_DOMAIN . url_wap("login#facebook_login");
        if (DEBUG_FB) {
            log_result('-获取脸书跳转jump_url-');
            log_result($jump_url);
        }
        $feacebook = new facebook($facebook_appid, $facebook_secrit, $jump_url);

        if ($_REQUEST['access_token'] != "") {

            $fb_info = $feacebook->sns_get_userinfo($_REQUEST['access_token']);
            $key = "fb_login_{$_REQUEST['access_token']}";

            if ($fb_info && intval($root['status']) != 0) {
                if (intval($fb_info['errcode']) != 0) {
                    $root['status'] = 0;
                    $root['error'] = $fb_info['errcode'] . $fb_info['errmsg'];
                    ajax_return($root);
                }

                if (DEBUG_FB) {
                    log_result('-脸书scope_get_userinfo->fb_info-');
                    log_result($fb_info);
                }
                fanwe_require(APP_ROOT_PATH . "system/libs/user.php");
                $root = facebookMakeUser($fb_info);
                if (DEBUG_FB) {
                    log_result('-登录信息facebookMakeUser-');
                    log_result($root);
                }

                if (empty($root['user_id'])) {
                    $GLOBALS['cache']->set($key, $fb_info, 300, true);
                    ajax_return($root);
                }
                ...
                //'0：微信；1：QQ；2：手机；3：微博 ;4 : 游客登录 ；5:脸书登录 ',
                $log_date['login_type'] = 5;
                $log_date['user_id'] = $root['user_id'];
                if (function_exists('log_login')) {
                    log_login($log_date);
                }
            ...
            if (DEBUG_FB) {
                log_result('-服务端获取参数失败-');
            }
            $root['status'] = 0;
            $root['error'] = "无法获取APP端参数(access_token)!";
        }
        ajax_return($root);
    }

    public function wx_login_h5()
    {
        $is_weixin = isWeixin();
        ...
        if ($wx_info['openid']) {
            ...
            if ($root['status'] == 1 && $root['need_bind_mobile'] == 1) {
                app_redirect(SITE_DOMAIN . '/frontEnd/weixin/index.html#/login/index');
        ...
        $back_url = SITE_DOMAIN . url_wap("login#wx_login_h5");
