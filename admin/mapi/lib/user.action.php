...
class userModule extends baseModule
{
    ...
    public function userinfo()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if (intval(OPEN_WAWA) == 1) {
                $root['has_billing'] = 1; //是否有账单明细
                $root['has_score'] = 1; //是否有积分
                $root['has_invitation'] = 1; //是否有邀请
            }
            ...
            $root['user']['tags'] = load_auto_cache('anchor_tag', array('user_id' => $to_user_id));
        }
    ...
    public function forbid_send_msg()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $guard = user_guard_syn_redis($to_user_id, $video['user_id']);
            if (!empty($guard)) {
                $root['error'] = "守护不能被禁言";
                ajax_return($root);
            }
    ...
    public function tipoff()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $remark = isset($_REQUEST['remark']) ? strim($_REQUEST['remark']) : ''; //备注
            $data = isset($_REQUEST['data']) ? json_decode($_REQUEST['data'], true) : null; //举报图片集合
            ...
            $tipoff['remark'] = $remark;
            $tipoff['data'] = json_encode($data);
    ...
    public function user_admin()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            foreach ($list as $k => $v) {
                ...
                $list[$k]['nick_name'] = ($list[$k]['nick_name']);
            }
            $root['list'] = $list;
    ...
    public function user_follow()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            foreach ($list as $k => $v) {
                ...
                $list[$k]['signature'] = ($list[$k]['signature']);
                $list[$k]['nick_name'] = ($list[$k]['nick_name']);
            }
            $root['list'] = $list;
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if (defined('OPEN_SHARE_EXPERIENCE') && OPEN_SHARE_EXPERIENCE == 1 && intval($m_config['open_share_ticket'])) {
                ...
                if ($share_id == 0 && intval($m_config['share_ticket']) > 0 && intval($video_user_info['from_user_id']) != $user_id && intval($video_share_id) > 0) {
                    //根据open_share_ticket决定获得秀票或秀豆 1秀票 2秀豆
                    ...
                    $GLOBALS['db']->query($sql); //增加用户秀票或秀豆
                    ...
                    $param['type'] = 4; //类型 0表示充值 1表示提现 2赠送道具 3 兑换秀票 4 分享获得秀票
    ...
    public function user_review()
    {
        ...
        if ($to_user_id == intval($GLOBALS['user_info']['id'])) {
            $sql = "select count(*)  from " . DB_PREFIX . "video_history as vh left join " . DB_PREFIX . "user as u on u.id= vh.user_id  where vh.group_id!='' and  vh.is_delete = 0 and vh.is_del_vod = 0 and vh.is_authorization =1 and vh.user_id = '" . $to_user_id . "' order by " . $sort_field;
        } else {
            $sql = "select count(*)  from " . DB_PREFIX . "video_history as vh left join " . DB_PREFIX . "user as u on u.id= vh.user_id  where vh.group_id!='' and  vh.is_delete = 0 and vh.is_del_vod = 0 and vh.is_authorization =1 and vh.is_live_pay = 0 and user_id = '" . $to_user_id . "' order by " . $sort_field;
        }
                ...
                if ($m_config['search_change'] == 0) {
                    if ($page == 1) {
                        if (intval($keyword) > 0) {
                            //如果搜索关键字为数字
                            //搜索ID，昵称,靓号，不为机器人。
                            $sql = "(select u.id as user_id,u.nick_name,u.signature,u.sex,u.head_image,u.user_level,u.v_icon,v.live_in from " . DB_PREFIX . "user u left join " . DB_PREFIX . "video as v on u.id = v.user_id where  u.id = " . $keyword . " and u.is_robot=0 limit 0,1)
                                union (select u.id as user_id,u.nick_name,u.signature,u.sex,u.head_image,u.user_level,u.v_icon,v.live_in from " . DB_PREFIX . "user u left join " . DB_PREFIX . "video as v on u.id = v.user_id where u.luck_num = " . $keyword . " and u.is_robot=0 limit 0,1)
                                union (select u.id as user_id,u.nick_name,u.signature,u.sex,u.head_image,u.user_level,u.v_icon,v.live_in from " . DB_PREFIX . "user u left join " . DB_PREFIX . "video as v on u.id = v.user_id where u.nick_name = '" . $keyword . "' and u.is_robot=0) limit 20";
                        } else {
                            $sql = "select u.id as user_id,u.nick_name,u.signature,u.sex,u.head_image,u.user_level,u.v_icon,v.live_in from " . DB_PREFIX . "user u left join " . DB_PREFIX . "video as v on u.id = v.user_id where  u.nick_name = '" . $keyword . "' and u.is_robot=0 limit " . $limit;
                        }
                    } else {
                        $sql = "select u.id as user_id,u.nick_name,u.signature,u.sex,u.head_image,u.user_level,u.v_icon,v.live_in from " . DB_PREFIX . "user u left join " . DB_PREFIX . "video as v on u.id = v.user_id where  u.nick_name = '" . $keyword . "' and u.is_robot=0 limit " . $limit;
                    }
                    ...
                    if ($page == 1) {
                        if (intval($keyword) > 0) {
                            ...
                            $sql = "select u.id as user_id,u.nick_name,u.signature,u.sex,u.head_image,u.user_level,u.v_icon,v.live_in from " . DB_PREFIX . "user u left join " . DB_PREFIX . "video as v on u.id = v.user_id where  u.nick_name like '%" . $keyword . "%' and u.is_robot=0 limit " . $limit;
                        }
                    } else {
                        $sql = "select u.id as user_id,u.nick_name,u.signature,u.sex,u.head_image,u.user_level,u.v_icon,v.live_in from " . DB_PREFIX . "user u left join " . DB_PREFIX . "video as v on u.id = v.user_id where  u.nick_name like '%" . $keyword . "%' and u.is_robot=0 limit " . $limit;
                    }
...
//            $sql = "select f.podcast_id as user_id from ".DB_PREFIX."focus f where f.user_id = ".$user_id;
            ...
            if (!empty($root['list'])) {
                for ($i = 0; $i < count($root['list']); $i++) {
                    ...
                    $root['list'][$i]['signature'] = ($root['list'][$i]['signature']);
                }
            }
     ...
     * @DateTime  2018-09-06T10:05:38+0800
    ...
    public function tag_list()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $user_id = intval($GLOBALS['user_info']['id']);
            $tag_list = load_auto_cache('tag_list');
            $to_user_id = intval($_REQUEST['to_user_id']); //被查看的用户ID
            if (empty($to_user_id)) {
                return ajax_return(array('status' => 0, 'error' => "主播ID不能为空"));
            }
            $my_selected = $GLOBALS['db']->getAll('SELECT tag_id as id,tag_name as name FROM `' . DB_PREFIX . 'user_tag_list` WHERE `from_user_id`=' . $user_id . ' AND `user_id` = ' . $to_user_id);
            $my_selected_tags = array_column($my_selected, 'id');
            foreach ($tag_list as &$value) {
                $value['is_selected'] = in_array($value['id'], $my_selected_tags) ? 1 : 0;
            }
            $root['tag_list'] = $tag_list;
            $root['my_selected'] = $my_selected;
        }
        ajax_return($root);
    }
     ...
     * [set_tag 设置标签]
     * @Author    ZiShang520@gmail.com
     * @DateTime  2018-09-06T11:54:16+0800
     * @copyright (c) ZiShang520 All Rights Reserved
     */
    public function set_tag()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $tag_list = load_auto_cache('tag_list');
            $to_user_id = intval($_REQUEST['to_user_id']); //被查看的用户ID
            $tag_ids = isset($_REQUEST['tag_ids']) ? array_slice(array_map('intval', array_filter(explode(',', $_REQUEST['tag_ids']))), 0, 3) : array(); //标签iD
            if ($to_user_id == $user_id) {
                return ajax_return(array('status' => 0, 'error' => "不能给自己设置标签"));
            }
            if (empty($to_user_id)) {
                return ajax_return(array('status' => 0, 'error' => "主播ID不能为空"));
            }
            if (!$GLOBALS['db']->query('DELETE FROM `' . DB_PREFIX . 'user_tag_list` WHERE `user_id`=' . $to_user_id . ' AND `from_user_id` = ' . $user_id)) {
                return ajax_return(array('status' => 0, 'error' => "清除TAG失败"));
            }
            if (!empty($tag_ids)) {
                $tag_names = $GLOBALS['db']->getAll('SELECT id,name FROM `' . DB_PREFIX . 'user_tags` WHERE `id` IN (' . implode(',', $tag_ids) . ') AND `is_effect` = 1');
                if (empty($tag_names)) {
                    return ajax_return(array('status' => 0, 'error' => "标签不存在"));
                }
                try {
                    foreach ($tag_names as $value) {
                        $GLOBALS['db']->autoExecute(DB_PREFIX . 'user_tag_list', array('user_id' => $to_user_id, 'from_user_id' => $user_id, 'tag_id' => $value['id'], 'tag_name' => $value['name']), 'INSERT', '', 'SILENT');
                    }
                } catch (\Exception $e) {
                    return ajax_return(array('status' => 0, 'error' => "设置TAG失败"));
                }
            }
        }
        ajax_return($root);
    ...
    /**
     * [prop_backpack 获取背包数据]
     * @Author    ZiShang520@gmail.com
     * @DateTime  2018-09-19T15:19:45+0800
     * @copyright (c) ZiShang520 All Rights Reserved
     * @return    [type] [description]
     */
    public function prop_backpack()
    {
        $root = array();
        $root['status'] = 1;
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $user_id = intval($GLOBALS['user_info']['id']);
            $backpack_list = $GLOBALS['db']->getAll('SELECT id AS b_id,prop_id,num FROM `' . DB_PREFIX . 'prop_backpack` WHERE `user_id` = ' . $user_id . ' AND num > 0');
            foreach ($backpack_list as $key => &$value) {
                $prop = load_auto_cache('prop', array('id' => $value['prop_id']));
                if (!empty($prop)) {
                    $value['num'] = intval($value['num']);
                    $value += $prop;
                } else {
                    unset($backpack_list[$key]);
                }
            }
            $root['status'] = 1;
            $root['error'] = "获取成功";
            $root['list'] = array_values($backpack_list);
        }
        ajax_return($root);
    }
}
