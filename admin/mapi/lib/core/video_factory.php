...
class VideoFactory
{
    ...
    public function GetVodRecordFiles($channel_id, $begin_time)
    {
        ...
        if ($m_config['video_type'] == 7) {
            fanwe_require(APP_ROOT_PATH . 'mapi/lib/core/video_ucloud.php');
            $service = new VideoUcloud($m_config);
            return $service->GetRecord($channel_id);
        }

        if ($m_config['video_type'] == 8) {
            fanwe_require(APP_ROOT_PATH . 'mapi/lib/core/video_wawza.php');
            $service = new VideoWawza($m_config);
            return $service->GetRecord($channel_id);
        }
    ...
    {
        $m_config = load_auto_cache('m_config');
        // 0:腾讯云互动直播, 1:腾讯云直播, 2:金山云，3:星域，4:千秀云，5:阿里云
        if ($m_config['video_type'] == 2) {
            fanwe_require(APP_ROOT_PATH . 'mapi/lib/core/video_ksyun.php');
            $service = new VideoKsyun($m_config);
    ...
    public function get_acc_sign($key, $stream_id, $len = 300)
    {
        ...
        $txTime = strtoupper(base_convert(time() + $len, 10, 16));
