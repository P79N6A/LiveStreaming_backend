 ...
 * 推送类型:push_type 当会员数超过10W时，建议也要使用专门的服务器来处理：全服推送 类型的 and u.push_type =0
 * @param int $min_fans 最小粉丝数
 * @param int $max_fans 最大粉丝数
 * @param array $push_type 推送类型
 *
 *
 * 推送数据存储优化方案[备用]
 ...
 *
 * 5、在fanwe_edu_push_anchor表中,可以直接添加fans_count字段，插入表数据时，固化
 *
 */
//服务器进程执行时间
ini_set("max_execution_time", 600);//服务器会在 60 秒后强行中止正在执行的程序

function push_notice($min_fans = 0, $max_fans = 0, $push_type = array(0, 1))
{
    $sql = "select pa.id, pa.push_type, pa.user_id, pa.nick_name, pa.ext_id,pa.type,pa.content from " . DB_PREFIX . "edu_push_anchor as pa 
				left join " . DB_PREFIX . "user as u on u.id = pa.user_id 		
				where pa.status = 0 and u.mobile != '13888888888' and  u.mobile != '13999999999' ";// pa.push_type = 0 and u.fans_count < 50000

    if ($min_fans > 0) {
        $sql .= ' and u.fans_count>=' . $min_fans;
    }
    if ($max_fans > 0) {
        $sql .= ' and u.fans_count<=' . $max_fans;
    }
    if (is_array($push_type) && count($push_type) > 0) {
        $sql .= ' and pa.push_type in(' . implode(',', $push_type) . ")";
    }
    ...
    foreach ($list as $k => $v) {
        ...
        $sql = "update " . DB_PREFIX . "edu_push_anchor set status = 1 where status = 0 and id = " . $v['id'];
        ...
        if ($GLOBALS['db']->affected_rows() == 1) {
            ...
            if ($v['push_type'] == 1) {
                ...
                $followd_list = $user_redis->followed_by($user_id);//获得粉丝列表,可能很大，如有上百W的粉丝，做推送时，需要独立的服务器来处理
                do {
                    $keys = array_splice($followd_list, 0, 4000);//一次读取4000条数据
            ...
            if (intval($num) > 10000) {
                ...
                foreach ($code_list as $ck => $cv) {
                    ...
                    //排除主播设备号
                    if ($my_apns_code == $apns_code || in_array($apns_code, $code_android_arr) || in_array($apns_code,
                            $code_ios_arr)
                    ) {
                        continue;
                    }
                ...
                if ($code_android_arr) {
                    ...
                    $sql = "update " . DB_PREFIX . "edu_push_anchor set ret_android_status = '" . $return['res']['ret'] . "', ret_android_data = '" . serialize($return['res']['data']) . "', android_file_id = '" . $return['file_id'] . "' where  id = " . $v['id'];
                ...
                if ($code_ios_arr) {
                    ...
                    $sql = "update " . DB_PREFIX . "edu_push_anchor set ret_ios_status = '" . $return['res']['ret'] . "', ret_ios_data = '" . serialize($return['res']['data']) . "', ios_file_id = '" . $return['file_id'] . "' where  id = " . $v['id'];
                    $GLOBALS['db']->query($sql);
                }
            } else {
                //得到机器码列表
                ...
                foreach ($code_list as $kk => $vv) {
                    ...
                    if ($i % 500 == 0 && $i != 0) {
                        ...
                        $sql = "update " . DB_PREFIX . "edu_push_anchor set  ret_android_status = '" . $return['res']['ret'] . "', ret_android_data = '" . serialize($return['res']['data']) . "' where  id = " . $v['id'];
                        $GLOBALS['db']->query($sql);
                        //重置机器列表
                        $i = 0;
                    ...
                    if ($j % 500 == 0 && $j != 0) {
                        ...
                        // ios_dest = '".$ios_dest."',
                        $sql = "update " . DB_PREFIX . "edu_push_anchor set ret_ios_status = '" . $return['res']['ret'] . "', ret_ios_data = '" . serialize($return['res']['data']) . "' where  id = " . $v['id'];
                        $GLOBALS['db']->query($sql);
                        //重置机器列表
                        $j = 0;
            ...
            //推送结束
            $sql = "update " . DB_PREFIX . "edu_push_anchor set status = 2 where status = 1 and id = " . $v['id'];
            $GLOBALS['db']->query($sql);
        }
    }
