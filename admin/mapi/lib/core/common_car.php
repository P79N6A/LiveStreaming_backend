    ...
    if ($room_id > 0) {
        ...
        if ($root['room_type'] == 1 && $root['user_id'] != $user_id) {
            ...
            if (defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE) {
                ...
                if ($private_code == 0 || $private_code == 'null') {
                    $root['error'] = "请输入邀请验证码！";
                    $root['status'] = 0;
                    ajax_return($root);
                }
...
//$has_scroll == 1 &&
    ...
    if ($root['id']) {
        ...
        fanwe_require(APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php');
        $user_redis = new UserRedisService();
        $user_info = $user_redis->getRow_db($user_id, array('id', 'nick_name', 'user_level', 'is_robot', 'is_authentication', 'luck_num', 'mobile'));
        ...
        if (($root['live_in'] == 1 && $is_vod == 0) || ($user_id == $root['user_id'] && $is_vod == 0) || (($root['live_in'] == 3 || $root['live_in'] == 0) && $is_vod == 1)) {
            ...
            $root['podcast']['user']['nick_name'] = ($root['podcast']['user']['nick_name']);
            ...
            if (defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE) {
                //等级颜色
                if ($user_info['user_level'] > 0 && $user_info['user_level'] <= 20) {
                    $root['podcast']['v_identity_colour'] = "粉红";
                } elseif ($user_info['user_level'] > 20 && $user_info['user_level'] <= 40) {
                    $root['podcast']['v_identity_colour'] = "青";
                } else {
                    $root['podcast']['v_identity_colour'] = "绿";
                }

                if ($user_info['is_authentication'] == 2) {
//会员
                    $root['podcast']['v_identity'] = 1;
                    $root['podcast']['v_identity_colour'] = "红";
                    $root['podcast']['v_join_name'] = "用户【" . $user_info['nick_name'] . "】-会员-" . $user_info['user_level'] . "级，进入房间";
                } else {
//游客
                    $root['podcast']['v_identity'] = 0;
                    $root['podcast']['v_join_name'] = "用户【" . $user_info['nick_name'] . "】-游客-" . $user_info['user_level'] . "级，进入房间";
                }

                //$root['user_id']主播id $user_id用户id
                $res = $GLOBALS['db']->getOne("select id from " . DB_PREFIX . "user_admin where user_id=$user_id and podcast_id=" . $root['user_id']);
                if (!empty($res)) {
//管理员
                    $root['podcast']['v_identity'] = 2;
                    $root['podcast']['v_identity_colour'] = "橙";
                    $root['podcast']['v_join_name'] = "用户【" . $user_info['nick_name'] . "】-管理员-" . $user_info['user_level'] . "级，进入房间";
                }
            ...
            if (defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE) {
                ...
                if ($allow_max_number > 0 && $root['viewer_num'] >= $allow_max_number && $root['user_id'] != $user_id) {
...
function createPropTable($time = NOW_TIME)
{
    ...
    if (!$res) {
        ...
        $sql = "CREATE TABLE `$table` (
          ...
          `is_rocket` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否火箭榜礼物 1是 0否',
          PRIMARY KEY (`id`),
