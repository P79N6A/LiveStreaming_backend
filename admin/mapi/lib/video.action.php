...
class videoModule extends baseModule
{
    ...
    public function end_video()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            $video = $GLOBALS['db']->getRow($sql, false, true);
    ...
    /**
     * 主播心跳监听，每30秒监听一次;监听数据：时间点，秀票数，房间人数
     */
    public function monitor()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            fanwe_require(APP_ROOT_PATH . 'mapi/lib/redis/VideoRedisService.php');
            $video_redis = new VideoRedisService();
            $fields = array('vote_number', 'watch_number', 'is_live_pay', 'live_pay_time', 'live_pay_type', 'live_fee', 'live_is_mention', 'robot_num', 'virtual_watch_number', 'group_id', 'channelid', 'pk_ticket');
            $video_number = $video_redis->getRow_db($room_id, $fields);
            $vote_number = intval($video_number['vote_number']); //获得秀票数
         ...
         * uc_cloud非标更新
         */
        if ($m_config['video_type'] == 7 && isset($m_config['uc_nonstandard']) && $m_config['uc_nonstandard'] == 1) {
            fanwe_require(APP_ROOT_PATH . 'mapi/lib/core/video_ucloud.php');
            $service = new VideoUcloud($m_config);
            $data = $service->Get($video_number['channelid']);
            //更新推拉流地址
            $sql = "UPDATE " . DB_PREFIX . "video SET `push_rtmp`= '" . $data['push_rtmp'] . "', `play_rtmp`='" . $data['play_rtmp'] . "', `play_flv`='" . $data['play_flv'] . "', `play_hls`='" . $data['play_hls'] . "' WHERE live_in =1 AND id = " . $room_id . " AND user_id = " . $user_id;
            $root['ucloud_api']['update'] = $GLOBALS['db']->query($sql);
            $root['ucloud_api']['redis'] = $video_redis->update_db($room_id, array('push_rtmp' => $data['push_rtmp'], 'play_flv' => $data['play_flv'], 'play_rtmp' => $data['play_rtmp'], 'play_hls' => $data['play_hls']));
        }
        ...
        if ((defined('OPEN_LIVE_PAY') && OPEN_LIVE_PAY == 1) && $live_pay) {
            ...
            $users = $GLOBALS['db']->getRow($sql, false, true);
        ...
        $root['live']['guard_num'] = (int) $guard->get_guarded_len($user_id);
        ...
        if (defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE) {
            ...
            $heat_rank = intval(get_user_rank(intval($user_id)));
            video_push_im($user_id, $room_id, $group_id, $heat_rank);
        }
        if ($GLOBALS['user_info']['id'] == $user_id) {
            $end_time = $GLOBALS['db']->getOne("select end_time from " . DB_PREFIX . "user where id = " . $user_id);
            if ($end_time != '' && $end_time <= get_gmtime()) {
                fanwe_require(APP_ROOT_PATH . "mapi/lib/core/common.php");
                $sql = "select id,user_id,max_watch_number,virtual_watch_number,robot_num,vote_number,group_id,room_type,begin_time,end_time,channelid,cate_id,video_vid,video_type from " . DB_PREFIX . "video where id = " . $room_id . " and user_id = " . $user_id;
                $video = $GLOBALS['db']->getRow($sql, false, true);
                $update = $GLOBALS['db']->query("UPDATE " . DB_PREFIX . "user set end_time=NULL WHERE id=" . $user_id);
                if ($update) {
                    $m_config = load_auto_cache("m_config");
                    $system_user_id = $m_config['tim_identifier']; //系统消息
                    $podcast_id = trim($GLOBALS['user_info']['id']);
                    $video_room_id = trim($_REQUEST['room_id']);
                    $ext = array();
                    $ext['type'] = 17;
                    $ext['desc'] = '直播时间已到';
                    $ext['room_id'] = $video_room_id;
                    #构造高级接口所需参数
                    $msg_content = array();
                    //创建array 所需元素
                    $msg_content_elem = array(
                    ...
                    if ($video && $ret['ActionStatus'] != 'OK') {
                        $result = do_end_video($video, $video['video_vid'], 0, $video['cate_id']);
                    }
                    if ($video['group_id'] != '' && $result) {
                        ...
                        $ext['room_id'] = $video_room_id; //直播ID 也是room_id;只有与当前房间相同时，收到消息才响应
                        //发送广播：直播结束
                        fanwe_require(APP_ROOT_PATH . 'system/tim/TimApi.php');
                        $api = createTimAPI();
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if (($dev_type == 'ios' || $dev_type == 'android')) {
                ...
                if ($video_private_redis->check_user_drop($room_id, $user_id)) {
                    return ajax_return(array('status' => 0, 'error' => '您已经被踢出,不能再加入'));
                }
                //强制升级不升级无法查看直播
                $status = 1;
                $m_config = load_auto_cache("m_config"); //初始化手机端配置
                ...
                if ($status == 1) {
                    ...
                    $advert = $GLOBALS['db']->getRow("SELECT image,url FROM " . DB_PREFIX . "index_image WHERE show_position=6");

                    // $advert =array('image'=>'https://htps.yunhairichu.com/public/attachment/201805/15/21/5afadfe090f18.jpg','url'=>"http://m.ele.me");
                    $root['advert'] = $advert;
    ...
    public function cont()
    {
        ...
        if ($room_id == 0 && $user_id == 0) {
            ...
            if ($room_id > 0) {
                ...
                $root = $video_con->get_video_contribute_week($user_id, $page, $page_size);
                $root['total_num'] = intval($root['total_ticket_num']);
                $root['v_icon'] = $root['user']['v_icon'];
                $root['user']['ticket'] = intval($root['user']['ticket']);
                //$sql = "select ticket from ".DB_PREFIX."user where id = ".$user_id;
    ...
    public function check_lianmai()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            //判断是否开启车行定制 -- 判断用户是否有权限发起连麦
            if (intval(defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE)) {
                $user = $GLOBALS['db']->getOne("select open_lianmai  from " . DB_PREFIX . "user where id = " . $user_id);
                if (intval($user) == 1) {
                    $re = array("error" => "您无法发起连麦，请联系客服", "status" => 0);
                    ajax_return($re);
                }
            }
            /*
     ...
     * @DateTime  2018-07-19T11:22:15+0800
    ...
    public function add_video()
    {
        if (!$GLOBALS['user_info']) {
            ...
            //判断是否开启车行定制
            if (intval(defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE)) {
                $sql = "SELECT allow_start_time,allow_end_time,open_private,is_authentication,is_ban,ban_time,mobile,login_ip,ban_type,apns_code,sex,ticket,refund_ticket,user_level,fans_count,head_image,thumb_head_image FROM " . DB_PREFIX . "user where id = " . $user_id;
            } else {
            ...
            $user = $GLOBALS['db']->getRow($sql, false, true);
            // $video_classified = intval($_REQUEST['video_classified']);
            $video_classified = $GLOBALS['user_info']['classified_id'];
            ...
            if (intval($m_config['video_limit_num']) > 0) {
                $num = $GLOBALS['db']->getOne("SELECT COUNT(id) as num FROM " . DB_PREFIX . "video WHERE live_in=1 OR live_in = 3", true, true);
                if ($num >= intval($m_config['video_limit_num'])) {
                    $re = array("error" => "直播间已达上限，您暂时不能开播哦", "status" => 0);
                    ajax_return($re);
                }
            }
            ...
            if (intval($user['is_ban']) == 0 && intval($user['ban_time']) < get_gmtime()) {
                ...
                $is_livepk = intval($_REQUEST['is_livepk']); // 是否开启PK
                ...
                //--车行定制start-- 判断用户是否有权限开启私密播与直播时间段限制
                if (defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE) {

                    if ($user['open_private'] == 1 && $is_private == 1) {

                        $re = array("error" => "您无法开启私密直播，请联系客服处理 ", "status" => 0);
                        ajax_return($re);
                    }
                    if ($user['allow_start_time'] != '' && $user['allow_end_time'] != '') {
                        $now = to_date(get_gmtime(), "H");
                        $to_day = 1;
                        if (intval($user['allow_start_time']) > intval($user['allow_end_time'])) {
                            $to_day = 0;
                        }
                        if ($to_day == 0 && intval($user['allow_start_time']) > $now && intval($user['allow_end_time']) <= $now) {
                            $re = array("error" => "请在每天的" . intval($user['allow_start_time']) . "时到第二天的" . intval($user['allow_end_time']) . "时期间进行直播", "status" => 0);
                            ajax_return($re);
                        }

                        if ($to_day == 1 && (intval($user['allow_start_time']) > $now || intval($user['allow_end_time']) <= $now)) {
                            $re = array("error" => "请在每天的" . intval($user['allow_start_time']) . "时到" . intval($user['allow_end_time']) . "时期间进行直播", "status" => 0);
                            ajax_return($re);
                        }
                    }
                }
                ...
                if ($is_private == 1) {
                    ...
                    if (defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE) {
                        $private_code = intval($_REQUEST['private_code']);
                        //测试用
                        if ($private_code == 0) {$private_code = rand(100000, 999999);} //私密直播code
                ...
                //车行定制 ljz 创建直播间时是否开启游客禁言
                if (defined('OPEN_CAR_MODULE') && OPEN_CAR_MODULE) {
                    $tourist_astrict = $_REQUEST['tourist_astrict'] ? $_REQUEST['tourist_astrict'] : 0;
                    $data['tourist_astrict'] = $tourist_astrict;
                }
                ...
                $data['is_livepk'] = $is_livepk; // 是否接受PK
                ...
                if (intval($user['is_ban'] && intval($user['ban_type'] == 0))) {
                    ...
                    $return['status'] = 0;
                    $return['error'] = '您目前还无法开启直播，截止到：' . to_date(intval($user['ban_time']), 'Y-m-d H:i:s') . '。';
                }
    ...
    public function check_status()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if ($video) {
                $video['nick_name'] = ($video['nick_name']);
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if ($video) {
                ...
                if (count($user_list) > 500) {
                    ...
                    //推送通知：
                    //推送消息文本
                    $nick_name = ($GLOBALS['user_info']['nick_name']);
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if ($video && $video['user_id'] != $user_id) {
                ...
                if (intval($GLOBALS['db']->getRow($sql)) == 0) {
                    //非管理员
                    unset($video);
                    ajax_return(array('status' => 0, 'error' => '没有权限'));
                }
            }
            ...
            if (!empty($video)) {
                ...
                if (count($user_ids) > 0) {
                    ...
                    foreach ($user_ids as $k => $v) {
                        ...
                        $guard = user_guard_syn_redis($v, $user_id);
                        if (!empty($guard)) {
                            ajax_return(array('status' => 0, 'error' => '守护不能被踢'));
                            continue;
                        }
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if (!empty($root['list'])) {
                for ($i = 0; $i < count($root['list']); $i++) {
                    $root['list'][$i]['nick_name'] = ($root['list'][$i]['nick_name']);
                }
            }
    ...
    public function video_list()
    {
        ...
        $param = array('page' => $page, 'page_size' => $page_size, 'cate_id' => $cate_id, 'is_recommend' => $is_recommend);
     ...
     * room_id:房间号id
     */
    public function check_private_code()
    {
        $root = array();
        if (!$GLOBALS['user_info']) {
            $root['error'] = "用户未登陆,请先登陆.";
            ...
            $room_id = intval($_REQUEST['room_id']); //房间号id
            $user_id = intval($GLOBALS['user_info']['id']); //用户ID
            $private_code = intval($_REQUEST['private_code']); //私密直播private_code

            if ($private_code != 0) {
                $sql = "select v.id,v.city,v.live_in,v.user_id,v.group_id,u.nick_name,u.head_image,v.video_type from " . DB_PREFIX . "video v left join " . DB_PREFIX . "user u on u.id = v.user_id where v.room_type = 1 and v.id = " . $room_id . " and v.private_code='" . $private_code . "'";
                $video = $GLOBALS['db']->getRow($sql);
