...
class appModule extends baseModule
{
    ...
    public function init()
    {
        ...
        //facebook登录
        $root['has_facebook_login'] = intval($m_config['has_facebook_login']); //支持facebook登录
        ...
        $root['monitor_second'] = 10; //主播心跳监听，每10秒监听一次;监听数据：时间点，秀票数，房间人数
        ...
        $root['app_name'] = strim($m_config['app_name']); //app名称  千秀直播
        ...
        $root['kf_qq'] = intval($m_config['kf_qq']);

        //开启O2O开关
        $root['o2o_open_goods'] = intval(O2O_OPEN_GOODS);

        //积分商城开关
        $root['open_score_shop'] = intval(OPEN_SCORE_SHOP);
        if ($root['open_score_shop'] == 1) {
            //积分兑换分类
            $root['doll_exchange_cate'] = array();
            $root['doll_exchange_cate'] = $GLOBALS['db']->getAll("select id,title from " . DB_PREFIX . "doll_exchange_cate where is_effect = 1 order by sort desc ", true, true);
            $root['doll_exchange_cate_1'] = [array("id" => "0", "title" => "虚拟")];
            $root['doll_exchange_cate'] = array_merge($root['doll_exchange_cate_1'], $root['doll_exchange_cate']);
        }
        //登录赠送积分值
        ...
        $h5_url = array(
            ...
            'url_share_index' => SITE_DOMAIN . APP_ROOT . '/mapi/index.php?ctl=user_center&act=share_profit_index' // 西藏青稞秀h5
        );
        if (SHOPPING_GOODS == 1) {
        ...
        $root['h5_url'] = $h5_url;
        $root['h5_logo'] = $m_config['h5_logo']; //H5页面logo
        ...
        $root['init_version'] = strval($m_config['init_version']); //手机端配置版本号
        //是否开启私信
        ...
        if ($video_classified) {
            $video_classified_new = array();
            $video_classified_new[] = ['classified_id' => 0, 'title' => "全部"];
            foreach ($video_classified as $k => $v) {
                $video_classified_new[] = $v;
            }
            $root['video_classified'] = $video_classified_new;
        }
        // }
        ...
        //     //汽车商城入口
        //     $root['is_car_shopping'] = $m_config['is_car_shopping']; //开关
        //     $root['url_car_shopping'] = $m_config['url_car_shopping']; //地址
        //     $root['car_shopping_name'] = $m_config['car_shopping_name'] ? $m_config['car_shopping_name'] : "车商人商城";
        //     //首页分类
        //     $root['car_classify'] = load_auto_cache('car_classify');
        //     //游客默认字数限制
        //     $root['tourist_astrict_num'] = 8;
        // }
        ...
        //是否开启验证码充值定制
        $root['open_recharge_code'] = intval(RECHARGE_CODE);

        $root['register_gift_diamonds'] = intval($m_config['register_gift_diamonds']);
        $root['open_login_send_diamonds'] = intval(OPEN_LOGIN_SEND_DIAMONDS);
        $root['open_register_gift'] = intval(OPEN_REGISTER_GIFT);

        $root['hide_network_notice'] = intval(HIDE_NETWORK_NOTICE);

        $root['server_time'] = bcmul(microtime(true), 1000);

        $root['pop_msg_money'] = $m_config['pop_msg_money']; //发送弹幕需要的秀豆

        $root['full_msg_money'] = $m_config['full_msg_money']; //全服喇叭需要的秀豆

        $root['full_msg_jump'] = intval($m_config['full_msg_jump']); //全服喇叭是都支持跳转

        $root['open_sign_in'] = intval($m_config['open_sign_in']);

        //是否首次
        /*if ($GLOBALS['user_info']) {
        $root['first_register']= es_session::get("first_register")?es_session::get("first_register"):0;
        es_session::set("first_register",0);
        log_result('first_register');
        log_result($GLOBALS['user_info']['id']);
        log_result($root['first_register']);
        }else{
        $root['first_register']=0;
        log_result('no_first_register');
        } */
     ...
     * @return array
     */
    public function version2($dev_type, $version, $sdk_version_name)
    {
    ...
    public function new_prop()
    {
        $root = array();
        $root['status'] = 1;
        $root['list'] = load_auto_cache('prop_group_list', array('display' => false));
        ajax_return($root);
    }
    ...
    public function plugin_status()
    {
        ...
        if ($user_id == 0) {
            ...
            if (defined('OPEN_PLUGIN') && OPEN_PLUGIN) {
                ...
                if ($plugin['class_name'] == 'pai' || $plugin['class_name'] == 'shop') {
                    if ($open_plugin['open_auction'] == 1) {
                        $root['error'] = "您无法开启购物竞拍，请联系客服";
                        $root['status'] = 0;
                        ajax_return($root);
                    }
                }
     ...
     * @DateTime  2018-08-09T17:56:09+0800
     * @copyright (c) ZiShang520 All Rights Reserved
     * @return    [type] [description]
     */
    public function servertime()
    {
        ajax_return(array('servertime' => bcmul(microtime(true), 1000)));
    }
}
