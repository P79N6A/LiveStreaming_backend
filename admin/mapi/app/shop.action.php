...
class shopCModule extends baseModule
{
    ...
    public function mystore()
    {
        ...
        $podcast = $user_redis->getRow_db($podcast_id, array('id'));
        if ($podcast['id'] === false) {
            $root = array('status' => 0, 'error' => '主播不存在');
        ...
        $page_size = 20;
        $limit = $page_size * ($p - 1) . "," . $page_size;
        $table = DB_PREFIX . 'pc_goods';
        $field = 'id,name,imgs,price,url,description,kd_cost';
        $sql = "SELECT $field FROM $table WHERE user_id = {$podcast_id} AND is_delete = 0 order by id desc limit {$limit}";
        $count = $GLOBALS['db']->getOne("SELECT COUNT(1) FROM $table WHERE user_id = {$podcast_id} AND is_delete = 0");
        ...
        if ($goods === false) {
            api_ajax_return(array('status' => 0, 'error' => '推荐商品为空'));
        } else {
            api_ajax_return(array('status' => 1, 'error' => '', 'list' => $goods, 'page' => $page->show()));
        }
    }
    ...
    public function add_goods()
    {
        ...
        $status = $GLOBALS['db']->autoExecute(DB_PREFIX . "pc_goods", $goods, "INSERT");
        if ($status) {
            $root['status'] = 1;
            $goods_id = $GLOBALS['db']->insert_id();
            $info = $GLOBALS['db']->getRow("SELECT * FROM " . DB_PREFIX . "pc_goods WHERE id=" . $goods_id);
            $result_data = $info;
        } else {
            $root['status'] = 0;
            $root['error'] = "添加商品失败";
    ...
    public function edit_goods()
    {
        ...
        $info = $GLOBALS['db']->getRow("SELECT * FROM " . DB_PREFIX . "pc_goods WHERE id=" . $goods['id'] . " and is_delete=0 and user_id=" . $user_id);

        if ($info) {
            $status = $GLOBALS['db']->autoExecute(DB_PREFIX . "pc_goods", $goods, $mode = 'UPDATE', "id=" . $id);
            if ($status) {
                $root['status'] = 1;
                $root['error'] = "修改商品成功";
                if ($goods['imgs'] != '') {
    ...
    public function del_goods()
    {
        ...
        if ($info) {
            $goods['is_delete'] = 1;//商品状态
            if ($GLOBALS['db']->autoExecute(DB_PREFIX . "pc_goods", $goods, $mode = 'UPDATE', "id=" . $goods['id'])) {
                $root['status'] = 1;
                $root['error'] = "删除成功";
            } else {
                $root['status'] = 10058;
