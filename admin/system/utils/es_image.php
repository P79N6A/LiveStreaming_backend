...
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
        if(app_conf("USER_VERIFY_STATUS")==1){
            $log_verifyName = es_session::get($verifyName);
            $GLOBALS['db']->autoExecute(DB_PREFIX."image_verify_code",array("verify_code"=>$log_verifyName,"create_time"=>get_gmtime(),"client_ip"=>get_client_ip()),"INSERT");
            //删除超过5分钟的验证码
            $n_time=get_gmtime()-300;
            $GLOBALS['db']->query("DELETE FROM ".DB_PREFIX."image_verify_code WHERE create_time <=".$n_time);
        }
