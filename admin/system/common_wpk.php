...
function check_sms_send($mobile)
{
    ...
    $mobile_sql = "select count(*) from " . DB_PREFIX . "mobile_verify_code where mobile = '" . $mobile . "'  and (" . NOW_TIME . ">$now_date and " . NOW_TIME . "<=$to_date)  ";
    ...
    $ip_sql = "select count(*) from " . DB_PREFIX . "mobile_verify_code where   client_ip ='" . $now_ip . "' and (" . NOW_TIME . ">$ip_now_hour and " . NOW_TIME . "<=$ip_to_date)  ";
    $ip_count = $GLOBALS['db']->getOne($ip_sql);
    $ip_count = intval($ip_count);
//    log_result('ip_sql:'.$ip_sql);
    //    log_result('ip_now_hour:'.to_date($ip_now_hour));
    //    log_result('ip_to_date:'.to_date($ip_to_date));
    if (($mobile_count > SMS_MOBILE_SEND_COUNT) && (SMS_MOBILE_SEND_COUNT > 0)) {
        $data['status'] = 0;
        $data['info'] = "验证码发送失败，当前手机号已超过今天限额";
        ajax_return($data);
        return false;
    }
    if (($ip_count > SMS_IP_SEND_COUNT) && (SMS_IP_SEND_COUNT > 0)) {
        $data['status'] = 0;
        $data['info'] = "验证码发送失败，当前ip已超过今天限额";
        ajax_return($data);
        return false;
    }
}
...
function wx_withdraw_cash($refund_id)
{
    ...
    $user_info = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "user where id=" . $msg_item['user_id']);
    ...
    if ($msg_item) {
        ...
        $partner_trade_no = $order_id . '_' . time() . rand(10000, 99999); //商户订单号
...
// curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (compatible; MSIE 5.01;
        ...
        if ($return['return_code'] == 'SUCCESS' && $return['result_code'] == 'SUCCESS') {
            $GLOBALS['db']->query("update " . DB_PREFIX . "user_refund set pay_log='已付款',is_pay = 3,partner_trade_no = '" . $partner_trade_no . "',pay_time = " . get_gmtime() . " where id = " . $order_id);
            $GLOBALS['db']->query("update " . DB_PREFIX . "payment set total_amount = total_amount + " . $msg_item['money'] . " where id = " . $payment_info['id']);
            return true;
        } else {
            $GLOBALS['db']->query("update " . DB_PREFIX . "user_refund set pay_log = '" . $return['return_msg'] . "',partner_trade_no = '" . $partner_trade_no . "' where id = " . $order_id);
            return false;
        }
    }
}
...
function deal_admin_nav($navs)
{
    ...
    if (!defined("OPEN_SLBGROUP") || OPEN_SLBGROUP == 0) {
...
{
    //log_file("user_leverl_syn_1",'user_leverl_syn');
    ...
    //log_file("user_leverl_syn_2",'user_leverl_syn');
...
//宽,高固定,以短边缩放 1e 剪裁
        ...
        $img_path = str_replace("./public/", get_domain() . "/public/", $img_path);
        return $img_path;
    }
}
...
//微信端 封装url
function url_wx($route = "index", $param = array())
{
