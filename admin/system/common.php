...
{
    if (!empty($mobile) && !preg_match("/^\+?((?:9[679]|8[035789]|6[789]|5[90]|42|3[578]|2[1-689])|9[0-58]|8[1246]|6[0-6]|5[1-8]|4[013-9]|3[0-469]|2[70]|7|1)(?:\W*\d){0,13}\d$/", $mobile)) {
        return false;
    } else {
        return true;
    }
...
function save_image_upload($upd_file, $key = '', $dir = 'temp', $whs = array(), $is_water = false, $need_return = false)
{
    ...
    foreach ($list as $lkey => &$item) {
        ...
        foreach ($whs as $tkey => $wh) {
            $item['thumb'][$tkey]['url'] = false;
            $item['thumb'][$tkey]['path'] = false;
            if ($wh[0] > 0 || $wh[1] > 0) //有宽高度
            {
                ...
                if ($thumb = $image->thumb($item['path'], $wh[0], $wh[1], $thumb_type)) {
                    $item['thumb'][$tkey]['url'] = $thumb['url'];
                    $item['thumb'][$tkey]['path'] = $thumb['path'];
                    if (isset($wh[3]) && intval($wh[3]) > 0) //需要水印
                    {
                        $paths = pathinfo($item['thumb'][$tkey]['path']);
                        $path = $paths['dirname'];
                        $path = $path . "/origin/";
                        if (!is_dir($path)) {
                        ...
                        @file_put_contents($path . $filename, @file_get_contents($item['thumb'][$tkey]['path']));
                        $image->water($item['thumb'][$tkey]['path'], $water_image, $alpha, $place);
                    }
                }
            }
        }
    ...
    if ($GLOBALS['distribution_cfg']['OSS_TYPE'] && $GLOBALS['distribution_cfg']['OSS_TYPE'] != "NONE") {
        if ($GLOBALS['distribution_cfg']['OSS_TYPE'] == "ES_FILE") {
            ...
            if ($is_unlink && intval($GLOBALS['distribution_cfg']['OSS_NO_SAVE_LOCALHOST']) == 0) {
                ...
                if (file_exists($file_path) && $info['status'] == 200) {
                    unlink($file_path);
                }
...
function update_sys_config()
{
    ...
    if (!file_exists($filename)) {
        ...
        $pconnect = false;
        $db = new mysql_db($dbcfg['DB_HOST'] . ":" . $dbcfg['DB_PORT'], $dbcfg['DB_USER'], $dbcfg['DB_PWD'], $dbcfg['DB_NAME'], $dbcfg['DB_CHARSET'], $pconnect);
        //end 定义DB
...
function deal_admin_nav($navs)
{
    ...
    if (!defined("OPEN_SOCIETY_MODULE") || OPEN_SOCIETY_MODULE == 0 || $m_config['society_pattern'] == 0) {
//添加公会模式开关
...
//非无抽成模式下关闭公会收益列表、公会提现列表
    ...
    if (!defined("OPEN_GAME_MODULE") || OPEN_GAME_MODULE == 0) {
        ...
        if (defined('OPEN_DIAMOND_GAME_MODULE') && OPEN_DIAMOND_GAME_MODULE == 1) {
            foreach ($navs['PlugIn']['groups']['gameconf']['nodes'] as $key => $value) {
                $navs['PlugIn']['groups']['gameconf']['nodes'][$key]['name'] = str_replace('金币', '秀豆', $value['name']);
            }
        }
    }
    ...
    //礼物中奖
    if (!defined('OPEN_REWARD_GIFT') || OPEN_REWARD_GIFT == 0) {
        unset($navs['PlugIn']['groups']['awardconf']);
    }
...
function user_leverl_syn($user_data)
{
    ...
    if (intval($user_data['user_level']) < $user_level['level']) {
        ...
        $mount_info = $GLOBALS['db']->getRow('SELECT mount_id,mount_day FROM ' . DB_PREFIX . 'speak_level WHERE begin_level <="' . $user_level['level'] . '" AND end_level>"' . $user_level['level'] . '" LIMIT 1');
        if (!empty($mount_info)) {
            $mount_ = load_auto_cache('mount_id', array('id' => $mount_info['mount_id']));
            if (!empty($mount_)) {
                user_mount_syn($user_data['id'], array('mount_id' => $mount_info['mount_id'], 'day_length' => $mount_info['mount_day']));
...
function anchor_leverl_syn($user_data)
{
    ...
    //$user_score = $user_data['score']+floor($user_data['online_time']/app_conf('ONLINETIME_TO_EXPERIENCE'));
    //$user_score = $user_data['score'];
    $user_ticket = $user_data['ticket'] + $user_data['refund_ticket'];
    //用户获取当前等级
    //旧的，获取等级
    // $user_current_level = $GLOBALS['db']->getRow("select point from ".DB_PREFIX."user_level where `level` = ".intval($user_data['user_level']),true,true);
    //$user_current_level = load_auto_cache("user_level_lv",array('lv'=>intval($user_data['user_level'])));
    ...
    //新的，通过redis获取等级
    $anchor_level_info = load_auto_cache("anchor_level");
    $anchor_level = null;
    foreach ($anchor_level_info as $v) {
        // 两层关系即可
        unset($anchor_level['next_level']);
        $v['next_level'] = $anchor_level;
        $anchor_level = $v;
        if ($v['ticket'] <= intval($user_ticket)) {
            break;
        }
    }

    //旧的，获取最高等级积分信息
    //$anchor_level = $GLOBALS['db']->getRow("select point,level from ".DB_PREFIX."anchor_level where point <=".intval($user_ticket)." order by point desc",true,true);
    if (intval($user_data['anchor_level']) < $anchor_level['level']) {
        //$user_data['anchor_level'] = intval($anchor_level['level']);
        $GLOBALS['db']->query("update " . DB_PREFIX . "user set anchor_level = " . $anchor_level['level'] . " where id = " . $user_data['id']);

        $data = array();
        $data['anchor_level'] = $anchor_level['level'];
        $user_redis->update_db($user_data['id'], $data);
        //$pm_content = "恭喜您，您已经成为".$anchor_level['name']."等级的会员！";
        //send_notify($user_data['id'], $pm_content, "account#point");
    }
    /*
    if($user_current_level['point']>$anchor_level['point'])
    {
    $user_data['anchor_level'] = intval($anchor_level['level']);
    $GLOBALS['db']->query("update ".DB_PREFIX."user set anchor_level = ".$user_data['anchor_level']." where id = ".$user_data['id']);

    $data = array();
    $data['anchor_level'] = $user_data['anchor_level'];
    $user_redis->update_db($user_data['id'],$user_data);
    //$pm_content = "很报歉，您的会员等级已经降为".$anchor_level['name']."！";
    //send_notify($user_data['id'], $pm_content, "account#point");
    }
     */

    $anchor_level['u_ticket'] = $user_ticket;
    return $anchor_level;
}
/**
 *更新会员等
        ...
        if ($GLOBALS['distribution_cfg']['NEW_OSS']) {
            if ($pos === 0) {
                ...
                if ($width == 0 && $height > 0) {
                    ...
                    if ($gen == 0) {
                        $format = "/resize,m_mfit,h_" . $height . ",w_" . $width;
                    }
//宽,高固定,以短边缩放 1e 不剪裁
                    else {
                        $format = "/resize,m_fill,m_mfit,h_" . $height . ",w_" . $width;
                    }
//宽,高固定,以短边缩放 1e 剪裁
                }
            ...
            if ($pos === 0) {
                ...
                if ($width == 0 && $height > 0) {
                    ...
                    if ($gen == 0) {
                        $format = $width . "w_" . $height . "h_0c_1e_1x";
                    }
                    //宽,高固定,以短边缩放 1e 不剪裁
                    else {
                        $format = $width . "w_" . $height . "h_1c_1e_1x";
                    }
                    //宽,高固定,以短边缩放 1e 剪裁
                }
...
function login_prompt($user_id)
{
        ...
        if ($login_send_score > 0) {
            ...
            if (intval($user_log_id) == 0) {
                ...
                if ($user['login_time'] < $now_time) {
                    ...
                    $user = $user_redis->getRow_db($user_id, array('id', 'score', 'online_time', 'user_level', 'ticket', 'refund_ticket', 'anchor_level'));
                    ...
                    $data['log_admin_id'] = 0;
                    $param['type'] = 5; //类型 0表示充值 1表示提现 2赠送道具 3 兑换秀票 4 分享获得秀票 5 登录赠送积分
    ...
    if (defined("OPEN_UPGRADE_PROMPT") && OPEN_UPGRADE_PROMPT == 1) {
        if ($upgrade_level) {
            ...
            if ($user['user_level'] >= $upgrade_level && $user["last_login_level"] < $user['user_level']) {
//等级大于等于升级提示并且上一次登录时的等级小于现在的等级
...
$content:日志内容
$file:默认日志文件名
$path:日志相对路径(相对根目录下面public里面的路径,两边无需加/)
$positionFile:打日志的文件
$positionLine:日志在该文件的第几行被触发
 */

function log_by_ch($content, $file = 'LOG', $path = 'LOG_BY_CH', $positionFile = '', $positionLine = '')
{
    $path = APP_ROOT_PATH . "/public/" . $path . "/"; //绝对路径
    if (!is_dir($path)) {
        @mkdir($path, 0777); //创建最高权限路径
    }
    $file = $path . $file . "_" . date('Ymd') . ".log"; //创建日志文件名
    if (is_array($content)) {
        $content = var_export($content, true); //对象型数据转为字符串变量
    }
    $seperate = '--------------------------------------------------------------';
    $position = '';
    if ($positionFile != '') {
        if ($positionLine != '') {
            $position = "文件名：" . $positionFile . "----" . "触发行数:" . $positionLine;
        } else {
            $position = "文件名：" . $positionFile;
        }

    }
    $content = $seperate . "\n" . $position . "\n" . "日志时间：" . date('Y-m-d H:i:s', time()) . "\n" . $content . "\n\n";
    file_put_contents($file, $content, FILE_APPEND | LOCK_EX); //写日志
}
