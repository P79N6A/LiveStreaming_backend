...
class Wswxpay_payment implements payment
{
    public function get_payment_code($payment_notice_id)
    {
        ...
        $pay['class_name'] = "Wswxpay";
        $pay['url'] = SITE_DOMAIN . APP_ROOT . '/mapi/index.php?ctl=pay&act=get_display_code&pay_code=Wswxpay&notice_id=' . $payment_notice_id;
        $pay['sdk_code'] = array("pay_sdk_type" => "fuwap", "config" => array(
            "url" => SITE_DOMAIN . APP_ROOT . '/mapi/index.php?ctl=pay&act=get_display_code&pay_code=Wswxpay&notice_id=' . $payment_notice_id,
            "is_wap" => 1
        ));
        return $pay;
    }
    ...
    {
        $payment = $GLOBALS['db']->getRow("select id,config from " . DB_PREFIX . "payment where class_name='Wswxpay'");
    ...
    public function display_code($payment_notice_id)
    {
        if ($payment_notice_id) {
            ...
            require_once APP_ROOT_PATH . 'system/payment/Wspay/Wspay.php';
            try {
                // $data_return_url = SITE_DOMAIN . APP_ROOT . '/callback/payment/wswxpay_response.php';
                $data_notify_url = SITE_DOMAIN . APP_ROOT . '/callback/payment/wswxpay_notify.php';
                $wswxpay = new Wspay([
