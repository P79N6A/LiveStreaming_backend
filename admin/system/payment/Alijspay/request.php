...
// 相关配置参数
    ...
    public function submitOrderInfo($date){
        ...
        $this->reqHandler->setParameter('service','pay.alipay.jspay');//接口类型：pay.alipay.jspay
        $this->reqHandler->setParameter('mch_id',$this->account);//必填项，商户号
		...
		$this->reqHandler->setParameter('notify_url',' ');//通知地址，商户需改为自己正式的
		$this->reqHandler->setParameter('callback_url','http://www.swiftpass.cn');
        $this->reqHandler->setParameter('nonce_str',mt_rand(time(),time()+rand()));//随机字符串，必填项，不长于 32 位
		//$this->reqHandler->setParameter('buyer_logon_id','520zwxq@sina.com');//买家支付宝账号
		//$this->reqHandler->setParameter('buyer_id','2088012499266531');//买家支付宝用户ID(我的) 
		//$this->reqHandler->setParameter('buyer_id','2088012499266531');
        ...
        if($this->pay->call()){
            ...
            if($this->resHandler->isTenpaySign()){
                ...
                if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                     return (array('pay_url'=>$this->resHandler->getParameter('pay_url'),
    ...
    public function submitRefund(){
        ...
        $this->reqHandler->setParameter('version',$this->version);
        $this->reqHandler->setParameter('service','unified.trade.refund');//接口类型：trade.single.refund
        $this->reqHandler->setParameter('mch_id',$this->account);//必填项，商户号，由威富通分配
        $this->reqHandler->setParameter('nonce_str',mt_rand(time(),time()+rand()));//随机字符串，必填项，不长于 32 位
        $this->reqHandler->setParameter('op_user_id',$this->account);//必填项，操作员帐号,默认为商户号
