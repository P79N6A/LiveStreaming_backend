...
class Gfpay_payment implements payment
{
    public function get_payment_code($payment_notice_id)
    {
        ...
        $pay = array();
		$pay['is_wap'] = 1;//
		$pay['class_name'] = "Gfpay";
		$pay['url'] =SITE_DOMAIN.APP_ROOT.'/mapi/index.php?ctl=pay&act=get_display_code&pay_code=Gfpay&notice_id='.$payment_notice_id;
		$pay['sdk_code'] = array("pay_sdk_type"=>"yjwap","config"=>
			array(
				"url"=>SITE_DOMAIN.APP_ROOT.'/mapi/index.php?ctl=pay&act=get_display_code&pay_code=Gfpay&notice_id='.$payment_notice_id,
				"is_wap"=>1
			)
		);
		log_file($pay,'gf');
	...
	public function display_code($payment_notice_id)
	{
		...
		if($payment_notice_id){
			...
			$data_notify_url = SITE_DOMAIN.APP_ROOT.'/callback/payment/gfpay_notify.php';
			$payType = '1';//wap支付 不支持支付宝
