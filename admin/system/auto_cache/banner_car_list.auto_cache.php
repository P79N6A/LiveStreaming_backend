...
class banner_car_list_auto_cache extends auto_cache
{
    private $key = "banner_car:list:";
        ...
        if ($list === false) {
            ...
            if (!$is_ok) {
                ...
                if ($dev_type == 'ios' && $m_config['ios_check_version'] != '' && $m_config['ios_check_version'] == $sdk_version_name) {
                    //$video_list = load_auto_cache("select_heat_video_check");
                    $video_list = load_auto_cache("select_heat_video");
                } else {
                    $video_list = load_auto_cache("select_heat_video");
                }
                foreach ($list as $k => $v) {
