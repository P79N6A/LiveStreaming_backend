        ...
        if ($list === false) {
            ...
            if (!$is_ok) {
                ...
                if ($has_is_authentication && $has_private == 0) {
                    $sql = "SELECT v.id AS room_id, v.sort_num, v.group_id, v.user_id, v.city, v.title, v.cate_id, v.live_in, v.in_livepk, v.video_type, v.create_type, v.room_type, (v.robot_num + v.virtual_watch_number + v.watch_number) as watch_number, u.head_image,u.thumb_head_image,v.live_image, v.xpoint,v.ypoint, u.v_type, u.v_icon, u.nick_name,u.user_level,v.is_live_pay" . $is_gather . ",v.live_pay_type,v.live_fee,u.create_time as user_create_time, u.luck_num FROM " . DB_PREFIX . "video v
                ...
                if ($rank['list']) {
                    foreach ($rank['list'] as $kk => $vv) {
                        $rank_arr[$vv['video_id']] = count($rank['list']) - $kk;
                    }
                }
                foreach ($list as $k => $v) {
                    ...
                    $list[$k]['rank'] = intval($rank_arr[$v['room_id']]);

                }
                array_multisort(array_column($list, 'live_in'), SORT_ASC, $list);
                array_multisort(array_column($list, 'rank'), SORT_DESC, $list);
