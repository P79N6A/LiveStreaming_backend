...
class video_live_recommend_auto_cache extends auto_cache
{
    ...
    public function load($hot_list)
    {
        ...
        if ($live_video === false) {
            ...
            foreach ($live_video as &$value) {
                $room = $video_redis->getRow_db($value['room_id'], array('channelid', 'begin_time', 'create_time'));
                $value['channelid'] = $room['channelid'];
                if ($value['live_in'] == 3) {
                    $file_info = load_auto_cache('video_file', array('id' => $value['room_id'], 'video_type' => $value['video_type'], 'channelid' => $room['channelid'], 'begin_time' => $room['begin_time'], 'create_time' => $room['create_time']));
                    $value['fileid'] = $file_info['file_id'];
                    $value['play_url'] = $file_info['play_url'];
                }
            }
