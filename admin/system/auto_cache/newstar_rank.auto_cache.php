...
class newstar_rank_auto_cache extends auto_cache{
    ...
    public function newstar_ceil()
    {
        ...
        $sql = "select u.id as user_id ,u.nick_name,u.v_type,u.v_icon,u.head_image,u.sex,u.user_level,sum(v.total_ticket) as use_ticket ,u.is_authentication from " . DB_PREFIX . "user as u INNER JOIN " . $table. " as v on u.id = v.to_user_id where TO_DAYS(NOW())-TO_DAYS(FROM_UNIXTIME(u.create_time)) <=30 and " . $where . " GROUP BY to_user_id order BY use_ticket desc limit " . $limit;
        ...
        //总榜
        $sql = "select u.id as user_id ,u.nick_name,u.v_type,u.v_icon,u.head_image,u.sex,u.user_level,sum(v.total_ticket) as use_ticket ,u.is_authentication from " . DB_PREFIX . "user as u INNER JOIN " . $table. " as v on u.id = v.to_user_id where u.is_effect=1 and TO_DAYS(NOW())-TO_DAYS(FROM_UNIXTIME(u.create_time)) <=30 and is_red_envelope = 0 GROUP BY to_user_id order BY use_ticket desc limit " . $limit;
