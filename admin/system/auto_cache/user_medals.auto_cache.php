<?php
//底部文章
class user_medals_auto_cache extends auto_cache
{
    private $key = "user_medals:list";
    public function load($param = array())
    {
        ...
        if (!empty($param['medal_id'])) {
            $medal_id = intval($param['medal_id']);
            $condition .= " AND um.medal_id={$medal_id}";
        }
        if ($no_expired) {
        ...
        if ($list === false) {
            ...
            if (!$is_ok) {
                ...
                // if ($list === false) {
                $sql = "SELECT um.*,m.`name`,m.`icon` FROM " . DB_PREFIX . "user_medals AS um INNER JOIN " . DB_PREFIX . "medals AS m ON m.id=um.medal_id WHERE um.user_id={$user_id} {$condition} ORDER BY um.end_time DESC";
                if (!empty($medal_id)) {
