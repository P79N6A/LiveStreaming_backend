...
class new_list_auto_cache extends auto_cache
{
    ...
    {
        if ($param['change'] == 1) {
            $sql = "SELECT v.id AS room_id, v.sort_num, v.group_id, v.user_id, v.city, v.title, v.cate_id, v.live_in, v.video_type, v.room_type, v.room_title, (v.robot_num + v.virtual_watch_number + v.watch_number) AS watch_number, u.head_image, u.thumb_head_image, v.xpoint, v.ypoint, u.v_type, u.v_icon, u.nick_name, u.user_level FROM " . DB_PREFIX . "video v LEFT JOIN " . DB_PREFIX . "user u ON u.id = v.user_id WHERE v.live_in IN (1 , 3) AND v.room_type = 3 and UNIX_TIMESTAMP(NOW())-from_unixtime(u.create_time) < 30*24*60 ORDER BY RAND() , u.create_time DESC LIMIT 6";

            $list = $GLOBALS['db']->getAll($sql, true, true);
            foreach ($list as &$v) {
                $v['live_image'] = get_live_image($v);
