        ...
        if ($playback === false) {
            ...
            if (sizeof($focus_user)) {
                ...
                $sql = "select v.id as room_id,v.group_id,v.user_id,v.city,v.title,v.end_time,v.begin_time,v.max_watch_number,v.video_vid,
					u.nick_name,u.head_image,u.thumb_head_image,u.v_type,u.v_icon,v.live_in from " . DB_PREFIX . "video_history as v
                ...
                foreach ($playback as &$v) {
                    ...
                    $v['playback_time'] = get_time_len($v['begin_time'], $v['end_time']);
                    $v['begin_time_format'] = format_show_date($v['begin_time']);
                    $v['head_image'] = get_spec_image($v['head_image']);
                    $v['thumb_head_image'] = get_spec_image($v['thumb_head_image']);
                    if ($max_watch_number > 10000) {
                        $v['watch_number_format'] = round($max_watch_number / 10000, 2) . "万";
                    } else {
                        $v['watch_number_format'] = $max_watch_number;
                    }
                    $v['max_watch_number'] = $max_watch_number;
                    if ($v['title'] == '') {
                        $v['title'] = "....";
                    }
