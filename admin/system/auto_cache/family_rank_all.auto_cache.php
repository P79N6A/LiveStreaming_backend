...
class family_rank_all_auto_cache extends auto_cache
{
    ...
    public function family_ceil($sort = 'user_count', $limit = 10, $type)
    {
        ...
        $sql = "SELECT f.id AS family_id, f.logo AS family_logo, f.name AS family_name, f.user_id, u.nick_name, f.create_time, f.user_count FROM " . DB_PREFIX . "family AS f, " . DB_PREFIX . "user AS u WHERE {$where} ORDER BY {$sort} DESC, f.contribution desc, f.video_time desc, f.create_time desc LIMIT {$limit}";

        $family = $GLOBALS['db']->getAll($sql);
        foreach ($family as &$v) {
            $v['family_url'] = url("family#info", array("family_id" => $v['family_id']));
            $v['v_icon'] = get_domain() . '/public/images/rank/rank_' . $v["family_level"] . '.png';
