        ...
        if ($list === false) {
            ...
            if (!$is_ok) {
                ...
                if ($rank_name == 'day') {
                    ...
                    //month
                    $sql = "select u.id as user_id,u.nick_name,u.v_type,u.v_icon,u.head_image,u.sex,u.user_level,sum(v.total_ticket) as ticket,u.is_authentication from " . $table . " as v LEFT JOIN " . DB_PREFIX . "user as u on u.id = v.to_user_id where u.is_effect=1 and v.create_ym=" . to_date(NOW_TIME, 'Ym') . " and v.is_red_envelope = 0 GROUP BY v.to_user_id order BY ticket desc limit " . $limit;
                } else if ($rank_name == 'week') {
                    $sql = "select u.id as user_id,u.nick_name,u.v_type,u.v_icon,u.head_image,u.sex,u.user_level,sum(v.total_ticket) as ticket,u.is_authentication from  " . $table . " as v LEFT JOIN " . DB_PREFIX . "user as u  on u.id = v.to_user_id where u.is_effect=1 and v.create_w = week(curdate()) and v.is_red_envelope = 0 GROUP BY v.to_user_id order BY ticket desc limit " . $limit;
                } else {
                    //all
