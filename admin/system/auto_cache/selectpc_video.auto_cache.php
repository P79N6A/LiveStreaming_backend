...
//            $is_ok =  $GLOBALS['cache']->set_lock($this->key);
            ...
            if ($has_is_authentication && $m_config['ios_check_version'] == '') {
                $sql = "SELECT v.id AS room_id, v.channelid, v.begin_time, v.create_time, v.play_url, v.play_flv, v.play_hls, v.sort_num, v.group_id, v.user_id, v.city, v.title, v.cate_id, v.live_in, v.video_type, v.room_type, (v.robot_num + v.virtual_watch_number + v.watch_number) as watch_number, v.live_image, u.head_image,u.thumb_head_image, v.xpoint,v.ypoint, u.v_type, u.v_icon, u.nick_name,u.user_level FROM " . DB_PREFIX . "video v
					LEFT JOIN " . DB_PREFIX . "user u ON u.id = v.user_id where v.live_in in (1,3) and u.is_authentication = 2 ";
            } elseif ($is_family_hot) {
                $sql = " SELECT v.id AS room_id, v.channelid, v.begin_time, v.create_time, v.play_url, v.play_flv, v.play_hls, v.sort_num, v.group_id, v.user_id, v.city, v.title, v.cate_id, v.live_in, v.video_type, v.room_type, (v.robot_num + v.virtual_watch_number + v.watch_number) as watch_number, v.live_image, u.head_image,u.thumb_head_image, v.xpoint,v.ypoint, u.v_type, u.v_icon, u.nick_name,u.user_level,f.id  FROM " . DB_PREFIX . "family as f , " . DB_PREFIX . "video v LEFT JOIN " . DB_PREFIX . "user u ON u.id = v.user_id where u.family_id = f.id and u.family_id >0 and f.status=1 and v.live_in in (1,3) ";
            ...
            foreach ($list as $k => $v) {
                ...
                if ($v['live_in'] == 3 && empty($v['play_url'])) {
