...
class edu_select_video_auto_cache extends auto_cache
{
    ...
    public function load($param)
    {
        ...
        $limit = strim($param['limit']); //获取范围个数
        ...
        if ($list === false) {
            ...
            if (!$is_ok) {
                ...
                if ($has_is_authentication && $has_private == 0) {
                    $sql = "SELECT v.id AS room_id, v.sort_num, v.group_id, v.user_id, v.city, v.title, v.cate_id, v.live_in, v.video_type, v.create_type, v.room_type, v.create_time, (v.robot_num + v.virtual_watch_number + v.watch_number) as watch_number, u.head_image,u.thumb_head_image,v.live_image, v.xpoint,v.ypoint,ev.tags, ev.is_verify,ev.deal_id,ev.edu_cate_id,ev.booking_class_id,u.v_type, u.v_icon, u.nick_name,u.user_level,v.is_live_pay,v.live_pay_type,v.live_fee,u.create_time as user_create_time FROM " . DB_PREFIX . "video v  LEFT JOIN " . DB_PREFIX . "user u ON u.id = v.user_id  LEFT JOIN " . DB_PREFIX . "edu_video_info as ev ON ev.video_id = v.id  where v.live_in in (1,3) and u.is_authentication = 2 and u.is_hot_on =0 and u.mobile != '13888888888' and u.mobile != '13999999999'  ";
                } else {
                    $sql = "SELECT v.id AS room_id, v.sort_num, v.group_id, v.user_id, v.city, v.title, v.cate_id, v.live_in, v.video_type, v.create_type, v.room_type, v.create_time, (v.robot_num + v.virtual_watch_number + v.watch_number) as watch_number, u.head_image,u.thumb_head_image,v.live_image, v.xpoint,v.ypoint,ev.tags, ev.is_verify,ev.deal_id,ev.edu_cate_id,ev.booking_class_id,u.v_type, u.v_icon, u.nick_name,u.user_level,v.is_live_pay,v.live_pay_type,v.live_fee,u.create_time as user_create_time FROM " . DB_PREFIX . "video v  LEFT JOIN " . DB_PREFIX . "user u ON u.id = v.user_id  LEFT JOIN " . DB_PREFIX . "edu_video_info as ev ON ev.video_id = v.id  where v.live_in in (1,3) and u.is_hot_on =0 and  u.mobile != '13888888888' and u.mobile != '13999999999'  ";
                }
                if ($is_classify) {
                ...
                foreach ($list as $k => $v) {
                    ...
                    $list[$k]['create_time_text'] = to_date($v['create_time'], 'Y-m-d H:i');
                    //子房间获取聊天组ID
                    if (defined('CHILD_ROOM') && CHILD_ROOM == 1) {
                        if (empty($v['group_id'])) {
                            fanwe_require(APP_ROOT_PATH . 'mapi/lib/ChildRoom.class.php');
                            $child_room = new child_room();
                            $parent_id = $child_room->parent_id($v['room_id']);
                            $list[$k]['group_id'] = $GLOBALS['db']->getOne("SELECT group_id FROM " . DB_PREFIX . "video WHERE id =" . $parent_id);
                        }
                    }
                }
