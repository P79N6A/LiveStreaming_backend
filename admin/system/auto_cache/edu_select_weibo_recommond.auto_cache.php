...
class edu_select_weibo_recommond_auto_cache extends auto_cache
{
        ...
        if ($list === false) {
            ...
            if (!$is_ok) {
                ...
                if ($user_id > 0) {
                    ...
                    $comment_ids = array_column($list, 'weibo_id');
                    $diggs  = $GLOBALS['db']->getAll("select weibo_id from ".DB_PREFIX."weibo_comment where user_id = ".$user_id." and type = 2 and weibo_id in (" . implode(',', $comment_ids) . ")");
                    $digg_ids = array_column($diggs, 'weibo_id');
                } else {
                    $order_list_array = array();
                    $digg_ids = array();
                }
                ...
                if (count($list) > 0) {
                    foreach ($list as $k => $v) {
                        $list[$k]['has_digg'] = in_array($v['weibo_id'], $digg_ids) ? 1 : 0;
