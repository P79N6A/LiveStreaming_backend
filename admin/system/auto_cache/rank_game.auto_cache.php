    ...
    public function load($param)
    {
        ...
        if ($list === false) {
            ...
            if (!$is_ok) {
                ...
                if ($rank_name == 'day') {
//day
                    $sql = "select u.id as user_id,u.nick_name,u.v_type,u.v_icon,u.head_image,u.sex,u.user_level,sum(v.money) as ticket,u.is_authentication from  " . $table . " as v LEFT JOIN " . DB_PREFIX . "user as u  on u.id = v.user_id where u.is_effect=1 and v.type = 2 and v.create_time>" . (NOW_TIME - 86400) . " and v.game_log_id in ( select id FROM " . DB_PREFIX . "game_log_history where game_id=" . $game_id . ") GROUP BY v.user_id order BY ticket desc limit " . $limit;
...
//all
                    ...
                    $sql = "select u.id as user_id,u.nick_name,u.v_type,u.v_icon,u.head_image,u.sex,u.user_level,sum(v.money) as ticket,u.is_authentication from  " . $table . " as v LEFT JOIN " . DB_PREFIX . "user as u  on u.id = v.user_id where u.is_effect=1  and v.type = 2 and v.game_log_id in ( select id FROM " . DB_PREFIX . "game_log_history where game_id=" . $game_id . ") GROUP BY v.user_id order BY ticket desc limit " . $limit;
                }
