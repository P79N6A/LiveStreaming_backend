...
//							$weibo = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."weibo where id = ".$weibo_id);
							if($payment_notice['type_cate']=='weixin'||$payment_notice['type_cate']=='reward'||$payment_notice['type_cate']=='chat'){
								...
								log_result('weibo:user_id:'.$weibo['user_id']);
								$weiboCont_redis = new WeiboContributionRedisService();
						...
						if ($diamonds > 0){
							...
							if ((defined('DISTRIBUTION_SCAN')&&DISTRIBUTION_SCAN==1))   //二级分销_充值后的秀票抽成
							{
								
								$sql = "select p_user_id from ".DB_PREFIX."user where id = ".$user_id;
								$p_user_id = $GLOBALS['db']->getOne($sql,true,true);	//获取分销上级ID
								if ($p_user_id!=0)		//确定为分销下级用户
								{
									//秀豆-秀票兑换比例
									$sql1 = "select val from ".DB_PREFIX."m_config where code = 'diamond_to_ticket_rate'";
									$diamond_to_ticket_rate = $GLOBALS['db']->getOne($sql1,true,true);
									
									//分销下级充值-上级抽成比例
									$sql2 = "select val from ".DB_PREFIX."m_config where code = 'distribution_recharge_rate'"; 	
									$distribution_recharge_rate = $GLOBALS['db']->getOne($sql2,true,true);
									//上级成员秀票抽成总额计算
									$diamonds = intval($payment_notice['diamonds']);
									$tickets_add = intval($diamonds*$diamond_to_ticket_rate*$distribution_recharge_rate);
									$sql = "update ".DB_PREFIX."user set ticket = ticket + ".$tickets_add." where id = ".$p_user_id;
									$GLOBALS['db']->query($sql);

									//写入用户操作日志
									fanwe_require(APP_ROOT_PATH.'system/common.php');
									$data = array();
				                    $data['ticket'] = $tickets_add;
				                    $data['log_admin_id'] = 0;
				                    $data['sub_id'] = $user_id;
				                    $param['type'] = 8;//类型 8表示分销上级抽成
				                    $log_msg ='由于下级分销成员'.$user_id.'充值，获取'.$tickets_add.'秀票抽成';
				                    account_log_com($data,$p_user_id,$log_msg,$param);
								}
							}

							if ((defined('SHARE_DISTRIBUTION')&&SHARE_DISTRIBUTION==1))   //浪花分销
							{
								$m_config = load_auto_cache("m_config");//初始化手机端配置
								$money = $payment_notice['diamonds'];
								// 后台是否开启分销开关
								if($m_config['share_distribution'] == 1 && $user_id > 0 && $money > 0) {
									$sql = "select share_up_id from ".DB_PREFIX."user where id = ".$user_id;
									//获取分销上级ID
									$to_user_id = $GLOBALS['db']->getOne($sql,true,true);
									//确定为分销下级用户
									if ($to_user_id!=0){
										//上级成员秀票抽成总额计算
										$share_distribution_ratio = $m_config['share_distribution_ratio'];
										//充值的获得的秀豆 * 0.01 * 分销比例
										$share_money = $money * 0.01 * $share_distribution_ratio;
										$table = DB_PREFIX . 'share_profit_log';
										//插入:分销日志
										$video_prop = array();
										$video_prop['from_user_id'] = $user_id;
										$video_prop['to_user_id'] = $to_user_id;
										$video_prop['create_date'] =to_date(NOW_TIME, 'Y-m-d');
										$video_prop['ticket'] = $share_money;
										...
										$video_prop['consume_money'] =$money;
										$result = $GLOBALS['db']->autoExecute($table,$video_prop,'INSERT');
										//如果分销日志插入成功,则执行分成操作
										if (intval($result) > 0) {
											$sql = "update ".DB_PREFIX."user set ticket = ticket + ".$share_money." where id = ".$to_user_id;
											$GLOBALS['db']->query($sql);
										}
									}
								}
