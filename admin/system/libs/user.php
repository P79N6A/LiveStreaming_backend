...
function save_user($user_data, $mode = 'INSERT', $update_status)
{
    ...
    if ($user_data['mobile'] != '' && !check_mobile(trim($user_data['mobile'])) && OPEN_YPSMS != 1) {
    ...
    //强制结束直播
    if (isset($user_data['end_time'])) {
        $end_time = strim($user_data['end_time']);
        $user['end_time'] = $end_time != '' ? to_timespan($end_time) : 0;

    }
    //允许直播起始时间
    if (isset($user_data['allow_start_time'])) {
        $user['allow_start_time'] = strim($user_data['allow_start_time']);
    }
    //允许直播结束时间
    if (isset($user_data['allow_end_time'])) {
        $user['allow_end_time'] = strim($user_data['allow_end_time']);
    }
    //浪花定制-分销上级ID
    if (isset($user_data['share_up_id'])) {
        $user['share_up_id'] = intval($user_data['share_up_id']);
    }
    //直播间最大人数限制
    if (isset($user_data['allow_max_number'])) {
        $user['allow_max_number'] = intval($user_data['allow_max_number']);
    }
    ...
    if (isset($user_data['province'])) {
        $user['province'] = strim($user_data['province']);
    }
    if (isset($user_data['city'])) {
        $user['city'] = strim($user_data['city']);
    }

    //车行定制 ljz 主播类别
    if (isset($user_data['anchor_sort_id'])) {
        $user['anchor_sort_id'] = strim($user_data['anchor_sort_id']);
    }
    ...
    if (isset($user_data['allow_game_plugin'])) {
        $user['allow_game_plugin'] = intval($user_data['allow_game_plugin']);
    }
    ...
    if (isset($user_data['is_authentication'])) {
        if (intval($user_data['is_authentication']) == 3 || intval($user_data['is_authentication']) == 1 || intval($user_data['is_authentication']) == 0) {
            ...
            if (isset($user_data['v_type']) && strim($user_data['v_type'])) {
                $user['v_type'] = intval($user_data['v_type']);
            }
        }
    }
...
function save_mobile_user($user_data, $mode = 'INSERT')
{
    ...
    if (!check_mobile(trim($user_data['mobile'])) && OPEN_YPSMS != 1) {
...
function modify_account($data, $user_id, $log_msg = '', $param = array())
{
    ...
    $log_msg1 = $update = $where = '';
    $user_info = $GLOBALS['db']->getRow('SELECT ticket,diamonds FROM ' . DB_PREFIX . 'user WHERE id = ' . $user_id);
    if ($ticket < 0) {
        $_ticket = abs($ticket);
        $update = ', ticket=ticket-' . $_ticket;
        $where = ' AND ticket >=' . $_ticket;
        $log_msg1 .= '扣除秀票:' . $_ticket;
    } else {
        $update = ', ticket=ticket+' . $ticket;
        $log_msg1 .= '增加秀票:' . $ticket;
    }
    if ($diamonds < 0) {
        $_diamonds = abs($diamonds);
        $sql = "update " . DB_PREFIX . "user set diamonds = diamonds - " . $_diamonds . " {$update} where diamonds >= " . $_diamonds . " {$where} and id =" . $user_id;
        $log_msg1 .= '，扣除秀豆:' . $_diamonds;
    } else {
        $sql = "update " . DB_PREFIX . "user set diamonds = diamonds + " . $diamonds . " {$update} where id =" . $user_id . $where;
        $log_msg1 = '，增加秀豆:' . $diamonds;
    }
    ...
    if ($GLOBALS['db']->affected_rows()) {
        ...
        $GLOBALS['db']->autoExecute(DB_PREFIX . "user_recharge_log", array('user_id' => $user_id, 'before_ticket' => $user_info['ticket'], 'before_diamonds' => $user_info['diamonds'], 'ticket' => $ticket, 'diamonds' => $diamonds, 'log_info' => $log_msg, 'log_time' => get_gmtime(), 'log_admin' => intval($adm_session['adm_id']), 'log_ip' => get_client_ip()), 'INSERT');
        return true;
    } else {
        return false;
    }
...
function do_login_user($user_id_or_mobile, $verify_code, $p_user_id)
{
    ...
    if ($user_id_or_mobile != '') {
        if ($user_id_or_mobile != '13888888888' && $user_id_or_mobile != '13999999999') {
            if (!check_mobile(trim($user_id_or_mobile)) && OPEN_YPSMS != 1) {
                $result['info'] = '手机格式错误';
                return $result;
            }
...
function check_user($field_name, $field_data)
{
    ...
    if ($field_name == 'mobile' && !check_mobile(trim($user_data['mobile'])) && OPEN_YPSMS != 1) {
        $field_item['field_name'] = 'mobile';
        $field_item['error'] = FORMAT_ERROR;
        $res['status'] = 0;
        $res['data'] = $field_item;
...
function wxxMakeUser($wx_info, $has_user = false)
{
    ...
    if ($wx_info['unionid'] != "" || $wx_info['openid'] != "") {
        ...
        if (!$has_user) {
            ...
            if (defined('OPEN_FORCE_MOBILE') && OPEN_FORCE_MOBILE == 1 && (!$has_user || empty($has_user['mobile']))) {
                $data['login_type'] = 2;
            } else {
                $data['login_type'] = 0;
            }
            ...
            $user_exist_sql = "select count(*) from " . DB_PREFIX . "user where wx_unionid = '" . $wx_info['unionid'] . "' and wx_openid = '" . $wx_info['openid'] . "'";
            if (intval($GLOBALS['db']->getOne($user_exist_sql)) == 0) {
                if (defined('OPEN_BM') && OPEN_BM) {
                    $bm_config = load_auto_cache("bm_config");
            ...
            if ($has_user['is_effect'] != 1) {
                ...
                return $return;
            } elseif ($GLOBALS['db']->getOne("SELECT login_ip FROM " . DB_PREFIX . "user WHERE is_ban = 1 and ban_type = 1 and login_ip like '%" . get_client_ip() . "%' and ban_type = 1 and is_ban =1")) {
                $return['status'] = 0;
        ...
        if ($user_info['id'] != '') {
            ...
            if (defined('OPEN_FORCE_MOBILE') && OPEN_FORCE_MOBILE == 1 && $user_info['login_type'] != 2) {
                $data['login_type'] = 2;
            } else {
                $data['login_type'] = 0;
            }
            ...
            $return['session_id'] = es_session::id();
    ...
    if ($wx_info['unionid'] != "" || $wx_info['openid'] != "") {
        ...
        if (!$has_user) {
            ...
            if ($wx_info['headimgurl'] != '') {
                ...
                $data['head_image'] = get_domain() . '/public/images/defaulthead.png';
            }
            $data['wx_openid'] = $wx_info['openid'];
            $data['wx_unionid'] = $wx_info['unionid'];
            $data['create_time'] = NOW_TIME;
    ...
    if ($qq_info['openid'] != "") {
        ...
        if (!$has_user) {
            ...
            if ($qq_info['figureurl_qq_1'] != '') {
                ...
                $data['thumb_head_image'] = get_domain() . '/public/images/defaulthead.png';
            }
...
function register_gift(&$user)
{
    ...
    if (defined('OPEN_REGISTER_GIFT') && OPEN_REGISTER_GIFT == 1) {
        ...
        if (intval($m_config['register_gift']) == 1) {
            ...
            es_session::set("first_register", 1);
        }
 ...
 * @return mixed
 */
function facebookMakeUser($fb_info, $has_user = false)
{
    if ($fb_info['unionid'] != "" || $fb_info['openid'] != "") {
        if (!$has_user && $fb_info['unionid']) {
            $has_user = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "user where fb_unionid = '" . $fb_info['unionid'] . "'");
        }
        if (!$has_user && $fb_info['openid']) {
            $has_user = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "user where fb_openid='" . $fb_info['openid'] . "'");
        }
        //===========add  start ===========
        fanwe_require(APP_ROOT_PATH . 'mapi/lib/redis/UserRedisService.php');
        $user_redis = new UserRedisService();
        //===========add  end ===========
        if (defined('OPEN_FORCE_MOBILE') && OPEN_FORCE_MOBILE == 1 && (!$has_user || empty($has_user['mobile'])) && empty($fb_info['mobile'])) {
            return array(
                'status' => 1,
                'error' => '请绑定手机账户',
                'need_bind_mobile' => 1
        ...
        if (!$has_user) {
            ...
            $data = array();
            $data['id'] = $user_id;
            $data['nick_name'] = htmlspecialchars_decode($fb_info['username']);
            if ($GLOBALS['db']->getOne("SELECT nick_name FROM " . DB_PREFIX . "user WHERE nick_name ='" . $data['nick_name'] . "'")) {
                //昵称唯一性
                $data['nick_name'] .= $user_id;
            }
            $data['is_effect'] = 1;
            if ($fb_info['headimgurl'] != '') {
                $head_image = strtr($fb_info['headimgurl'], array('/0' => '/96'));
                $data['head_image'] = $head_image;
            } else {
                $m_config = load_auto_cache("m_config"); //初始化手机端配置
                $system_head_image = $m_config['app_logo'];
            ...
            $data['wx_openid'] = $fb_info['openid'];
            $data['wx_unionid'] = $fb_info['unionid'];
            $data['create_time'] = NOW_TIME;
            $data['user_pwd'] = md5(rand(100000, 999999));
            $data['login_ip'] = CLIENT_IP;
            $data['synchronize'] = 0;
            ...
            if ($fb_info['sex'] == 1) {
                $data['sex'] = 1;
            } elseif ($fb_info['sex'] == 2) {
                $data['sex'] = 2;
            } else {
                $data['sex'] = 1;
            }

            if (intval($GLOBALS['db']->getOne("select count(*) from " . DB_PREFIX . "user where wx_unionid = '" . $fb_info['unionid'] . "'")) == 0) {
                if (defined('OPEN_BM') && OPEN_BM) {
                    $bm_config = load_auto_cache("bm_config");
                    $data['bm_pid'] = intval($bm_config['bm_pid']);
                }
            ...
            if ($has_user['is_effect'] != 1) {
                ...
                if (!strpos($has_user['head_image'], 'wx.qlogo.cn') && intval($has_user['is_replace_wx']) == 0) {
                    $has_user['head_image'] = strtr($fb_info['headimgurl'], array('/0' => '/96'));
                    $sql = "update " . DB_PREFIX . "user set head_image = '" . $has_user['head_image'] . "',is_replace_wx=1 where id =" . $has_user['id'] . " and is_replace_wx = 0";
                    $GLOBALS['db']->query($sql);
                }
                $user_info = $has_user;
        ...
        if ($user_info['id'] != '') {
            ...
            $data['login_time'] = to_date($now_time);
            $data['wx_unionid'] = $fb_info['unionid'];
            if (empty($user_info['mobile']) && $fb_info['mobile']) {
                $data['mobile'] = $wx_info['mobile'];
            }
