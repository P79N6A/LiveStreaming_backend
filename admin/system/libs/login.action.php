...
class loginModule  extends baseModule
{
	...
	{
		if(trim($_REQUEST['mobile']) == '13888888888'){
			...
			if($result['status'])
			{
                ...
                $is_effect=$GLOBALS['db']->getOne("SELECT is_effect FROM ".DB_PREFIX."user WHERE id = $id");
                if($root['is_lack']){
                    $root['error'] = "请更新个人信息";
                }elseif($is_effect!=1){
                    ajax_return(array("status"=>0,"error"=>'账号已被禁用'));
                }elseif($GLOBALS['db']->getOne("SELECT login_ip,apns_code FROM ".DB_PREFIX."user WHERE login_ip like '%".get_client_ip()."%' and is_effect !=1")){
                    ajax_return(array("status"=>0,"error"=>'当前IP以被封停'));
                }else{
                    $root['error'] = "登录成功";
                }
				$root['user_info'] = $result['user_info'];
	...
	public function do_update()
	{
		...
		if($user_data['id']==''){
			ajax_return(array("status"=>0,"error"=>'ID不能为空'));
		}
        if($GLOBALS['db']->getOne("SELECT login_ip FROM ".DB_PREFIX."user WHERE login_ip like '%".get_client_ip()."%' and is_effect !=1")){
            ajax_return(array("status"=>0,"error"=>'当前IP以被封停'));
        }
	
		if($user_data['oss_path']==''){
			...
			$user_data['head_image']=$user_data['oss_path'];
		}
		...
		$user_head_image = $GLOBALS['db']->getOne("select head_image from ".DB_PREFIX."user where id =".$user_data['id']);
		
		if($open_sts&&$user_head_image==''){
			if($user_data['head_image']==''){	
				if($system_head_image==''){
					$system_head_image = './public/attachment/test/noavatar_10.JPG';
					syn_to_remote_image_server($system_head_image);
				}
				$user_data['head_image']=$system_head_image;
			}
		}else{
			if($user_head_image==''){
				if($system_head_image==''){
