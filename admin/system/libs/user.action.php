...
class userModule extends baseModule
{
    ...
    public function follow()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if (!$user_redis->getOne_db($to_user_id, 'id')) {
                $root['error'] = "关注的用户不存在！";
                $root['status'] = 0;
    ...
    public function share()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if (defined('OPEN_SHARE_EXPERIENCE') && OPEN_SHARE_EXPERIENCE == 1 && intval($m_config['open_share_ticket'])) {
                ...
                $sql = "select id from " . DB_PREFIX . "video_share  where video_id = " . $room_id . " and user_id =" . $user_id . " and type = '" . $type . "'";
            ...
            if (defined('OPEN_SHARE_EXPERIENCE') && OPEN_SHARE_EXPERIENCE == 1 && intval($m_config['open_share_ticket'])) {
                ...
                if ($share_id == 0 && intval($m_config['share_ticket']) > 0 && intval($video_user_info['from_user_id']) != $user_id && intval($video_share_id) > 0) {
                    ...
                    $log_msg = '分享直播间' . $room_id . '，获得' . intval($m_config['share_ticket']) . $ticket_name;
    ...
    public function user_review()
    {
        ...
        if ($to_user_id == intval($GLOBALS['user_info']['id'])) {
            $sql = "select id,title,begin_time,max_watch_number,video_vid from " . DB_PREFIX . "video_history where is_delete = 0 and is_del_vod = 0 and user_id = '" . $to_user_id . "' order by " . $sort_field . " limit " . $limit;
        } else {
            $sql = "select id,title,begin_time,max_watch_number,video_vid from " . DB_PREFIX . "video_history where is_delete = 0 and is_del_vod = 0 and is_live_pay = 0 and user_id = '" . $to_user_id . "' order by " . $sort_field . " limit " . $limit;
        }
        ...
        $sql = "select count(*) as num from " . DB_PREFIX . "video_history where is_delete = 0 and is_del_vod = 0 and user_id = '" . $to_user_id . "'";
...
//如果搜索关键字为数字
                    //搜索ID，昵称,靓号，不为机器人。
                    $sql = "select u.id as user_id,u.nick_name,u.signature,u.sex,u.head_image,u.user_level,u.v_icon from " . DB_PREFIX . "user u where u.id = '" . $keyword . "' or u.luck_num = '" . $keyword . "' or u.nick_name like '%" . $keyword . "%' and u.is_robot=0 limit " . $limit;
                } else {
                    $sql = "select u.id as user_id,u.nick_name,u.signature,u.sex,u.head_image,u.user_level,u.v_icon from " . DB_PREFIX . "user u where u.id = '" . $keyword . "' or u.nick_name like '%" . $keyword . "%' and u.is_robot=0 limit " . $limit;
                }
    ...
    public function state_change()
    {
        ...
        if (!$GLOBALS['user_info']) {
            ...
            if ($action == 'Login') {
                ...
                $data = array('is_online' => 0, 'logout_time' => to_date(NOW_TIME));
